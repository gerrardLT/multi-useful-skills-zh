执行系统化的**技术性**质量检查，并生成一份完整报告。不要在此处修复问题——将问题记录下来，交由其他命令处理。

这是一次代码层面的审计，而非设计评审。检查那些可测量、可验证的实现问题。

## 诊断扫描

围绕 5 个维度进行全面检查。每个维度按下方标准打 0-4 分。
...
### 反模式判定
**从这里开始。** 通过 / 失败：这看起来像 AI 生成的吗？把具体痕迹列出来。要足够严格。
...
给每个问题打上 **P0-P3 严重级别**：
- **P0 阻塞**：会阻止任务完成——必须立刻修复
- **P1 重大**：会造成显著困难，或违反 WCAG AA——应在发布前修复
- **P2 次要**：会带来烦扰，但有绕行方式——应在下一轮修复
- **P3 打磨**：锦上添花，不影响真实用户完成任务——有空再修复
...
> 您可以要求我一次运行一个、一次全部运行，或按您喜欢的任何顺序运行。
>
