...
- **Stitch 会将颜色校验为 hex sRGB**（`#RGB` / `#RGBA` / `#RRGGBB` / `#RRGGBBAA`）；OKLCH / HSL / P3 会触发 linter 警告，但不是硬错误。YAML 无论哪种格式都接受字符串，我们自己的 parser 也不限定格式。根据项目姿态来选择：a）如果项目坚持“仅 OKLCH”或使用无法可靠 round-trip 到 sRGB 的 Display-P3 值，那就直接在 frontmatter 里写 OKLCH，并接受 Stitch 的警告；b）如果项目更在意严格通过 Stitch 校验，或计划使用他们的 Tailwind / DTCG 导出链路，那就在 frontmatter 中写 hex，并在 prose 中把 OKLCH 作为规范参考。没有明确理由时，绝不要把事实来源拆成两份。
- **组件子 token** 只允许 8 个属性：`backgroundColor`、`textColor`、`typography`、`rounded`、`padding`、`size`、`height`、`width`。阴影、动效、focus ring、backdrop-filter 等都放不进去，应该放在 sidecar（第 4b 步）。
...
如果已经存在 `DESIGN.md`，**不要静默覆盖**。先把现有文件展示给用户，并询问是刷新、覆盖还是合并。
...
### Primary
- **[Descriptive Name]** (#HEX / oklch(...)): [在哪里以及为什么使用这种颜色。具体说明上下文，而不仅仅是角色。]

### Secondary (optional - 如果项目只有一个强调色则省略)
- **[Descriptive Name]** (#HEX): [角色。]

### Tertiary (optional)
- **[Descriptive Name]** (#HEX): [角色。]

### Neutral
- **[Descriptive Name]** (#HEX): [文本/背景/边框/分隔线角色。]
- [...]

### 已命名的 Rules (optional, powerful)
**The [Rule Name] Rule.** [简短、有力的禁止或原则——例如“The One Voice Rule. The primary accent is used on <=10% of any given screen. Its rarity is the point.”]
...
**Character:** [1-2 句话描述字体配对的个性。]

### Hierarchy
- **Display** ([weight], [size/clamp], [line-height]): [目的——出现在哪里。]
- **Headline** ([weight], [size], [line-height]): [目的。]
- **Title** ([weight], [size], [line-height]): [目的。]
- **Body** ([weight], [size], [line-height]): [目的。如果相关，包含最大行宽如 65-75ch。]
- **Label** ([weight], [size], [letter-spacing], [case if uppercase]): [目的。]

### 已命名的 Rules (optional)
**The [Rule Name] Rule.** [关于字体使用的简短原则。]

## 4. Elevation

[一段话：这套系统使用阴影、tonal layering 还是混合方式？如果“没有阴影”，请明确说明，并描述如何传达深度。]
...
- **Hover / Focus:** [过渡、处理方式]
- **Secondary / Ghost / Tertiary (if applicable):** [简要描述]
...
- **Corner Style:** [圆角]
- **Background:** [使用的颜色]
- **Shadow Strategy:** [引用 Elevation 章节]
- **Border:** [如果有]
- **Internal Padding:** [比例]
...
### [Signature Component] (optional - 如果项目有一个值得记录的独特自定义组件)
[描述。]

## 6. Do's and Don'ts

具体、有力的防护栏。每条以“Do”或“Don't”开头。要具体——包含精确的颜色、像素值，以及用户在 PRODUCT.md 中提到的命名反模式。**PRODUCT.md 中的每个 anti-reference 都应在这里以相同的语言作为“Don't”出现**，这样视觉规范就能贯穿战略线。尽可能直接引用 PRODUCT.md：如果 PRODUCT.md 说 *“avoid dark mode with purple gradients, neon accents, glassmorphism”*，这里的 Don'ts 就应该按名称重复。
...
### Don't:
- **Don't** [具体禁止——例如“use border-left greater than 1px as a colored stripe”]
- **Don't** [...]
- **Don't** [...]
```
...
- **签名型组件**（如果足够有辨识度就应该包含）：hero CTA、featured card、filter pill、以及 PRODUCT.md 里用户明确提到的重要自定义模式。
...
5. **一个 anti-reference。** 它绝对不应该是像什么感觉。也必须是具名对象。
