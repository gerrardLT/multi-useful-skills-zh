...
如果 DESIGN.md 缺失：每个会话只提醒一次（*"运行 `$impeccable document` 以获得更符合品牌风格的输出"*），然后继续。
...
适用于所有设计，无论注册类型是什么。让实现复杂度与审美目标相匹配，极繁需要更复杂的代码，极简需要更高精度。要有创造性地理解，不同项目必须做出不同选择，绝不要收敛成同一套答案。GPT 能做出非常出色的作品，不要收着。
...
- 在选颜色之前，先确定 **颜色策略**。它沿着承诺度轴分成四档：
  ...
  - **全色板**：有 3–5 个命名角色色，并且都被有意使用。适用于品牌活动页和产品中的数据可视化。
  - **浸染型**：表面本身就是颜色。适用于品牌 hero 和活动页面。
- “一个强调色不超过 10%” 只适用于克制型。承诺型 / 全色板 / 浸染型本来就会主动超过这个比例。不要条件反射地把所有设计都压回克制型。
...
如果有人看一眼这个界面，就能毫无疑问地说出“这就是 AI 做的”，那它就失败了。跨注册类型的失败信号就是上面的那些绝对禁令。各自注册类型的特定失败模式写在各自的参考里。
...
到这一步为止，设置（上下文收集、注册类型识别）应当已经完成；子命令不应再次调用 `$impeccable`。
