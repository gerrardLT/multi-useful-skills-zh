在项目根目录生成一个 `DESIGN.md` 文件，以捕获当前的视觉设计系统，使 AI 代理在生成新界面时能保持品牌一致性。
...
- **Seed 模式**：项目尚未实现（刚做完 teach，几乎没有代码）。通过五个高层问题写一份最小版 DESIGN.md，并标记 `<!-- SEED -->`。等以后有代码了，再用扫描模式重跑一遍。
...
- **Colors**：按主色 / 次色 / 第三色 / 中性色分组（这是 Stitch 采用的 Material 衍生角色体系）。如果项目只有一个强调色，那就写成主色 + 中性色，不要凭空发明次色 / 第三色。
...
- **Components**：每个 variant 一个条目（`button-primary`、`button-primary-hover`、`button-ghost`）。通过 `{colors.X}`、`{rounded.Y}` 引用基础 token。如果某个 variant 需要 Stitch 那 8 个属性之外的内容（如 shadow、focus ring、backdrop-filter），就在 sidecar 里承载完整片段。
...
- **Overview 语气**：氛围形容词、2-3 句的审美哲学、anti-references（这套系统不应给人什么感觉）。
- **Color character**（针对自动提取出的颜色）：为关键色提供描述性命名（如 “Deep Muted Teal-Navy”，而不是 “blue-800”）。根据色相 / 饱和度，为每个关键色提供 2-3 个备选名。
- **Elevation 哲学**：是 flat / layered / lifted？如果存在阴影，它们的角色是 ambient 还是 structural？
...
[2-3 段整体描述：个性、密度、审美哲学。从 North Star 开始向外展开。说明此系统明确拒绝什么（从 PRODUCT.md 的 anti-references 中提取）。以简短的 **Key Characteristics:** 要点列表结束。]
...
### 主色
- **[描述性名称]** (#HEX / oklch(...)): [这种颜色在什么场景使用、为什么使用。要写清具体上下文，不要只写角色名。]

### 次色（可选 - 如果项目只有一个 accent，就省略）
- **[描述性名称]** (#HEX): [角色。]
...
### 中性色
- **[描述性名称]** (#HEX): [用于 text / background / border / divider 的角色。]
- [...]

### 具名规则（可选，但很有力）
**The [Rule Name] Rule.** [简短、有力的禁令或信条 - 例如 "The One Voice Rule. The primary accent is used on <=10% of any given screen. Its rarity is the point."]

## 3. Typography

**Display Font:** [字体家族]（带 [fallback]）
**Body Font:** [字体家族]（带 [fallback]）
**Label/Mono Font:** [字体家族，如有单独配置]

**Character:** [1-2 句话描述此字体搭配的个性。]

### 层级
- **Display** ([weight], [size/clamp], [line-height]): [用途 - 出现在哪里。]
- **Headline** ([weight], [size], [line-height]): [用途。]
- **Title** ([weight], [size], [line-height]): [用途。]
- **Body** ([weight], [size], [line-height]): [用途。如果相关，包含最大行宽如 65-75ch。]
- **Label** ([weight], [size], [letter-spacing], [case if uppercase]): [用途。]

### 具名规则（可选）
**The [Rule Name] Rule.** [关于字体使用的简短信条。]

## 4. Elevation

[一段话：此系统使用阴影、色调分层还是混合？如果“无阴影”，请明确说明，并描述如何传达深度。]

### 阴影视觉词汇（如适用）
- **[Role name]** (`box-shadow: [exact value]`): [何时使用。]
- [...]

### 具名规则（可选）
**The [Rule Name] Rule.** [例如 "The Flat-By-Default Rule. Surfaces are flat at rest. Shadows appear only as a response to state (hover, elevation, focus)."]
...
### 按钮
- **Shape:** [对圆角/形状的描述，括号里写精确数值]
- **主按钮：** [颜色分配 + padding，同时写语义描述和精确值]
- **Hover / Focus:** [过渡与处理方式]
- **次级 / Ghost / 第三层级（如适用）：** [简短描述]

### Chips（如使用）
- **Style:** [background、text color、border 处理]
- **State:** [selected / unselected，以及 filter / action variants]

### 卡片 / 容器
- **Corner Style:** [radius]
- **Background:** [所用颜色]
- **Shadow Strategy:** [参考 Elevation 一节]
- **Border:** [如果有]
- **Internal Padding:** [尺度]

### 输入框 / 表单字段
- **Style:** [stroke、background、radius]
- **Focus:** [处理方式 - glow、border shift 等]
- **Error / Disabled:** [如适用]

### 导航
- **样式、排版、default/hover/active 状态，以及移动端处理方式。**

### [Signature Component]（可选 - 如果项目里有值得单独记录的标志性自定义组件）
[描述。]

## 6. Do's and Don'ts

具体、强硬的护栏规则。每条都以 “Do” 或 “Don't” 开头。一定要具体，写出准确颜色、像素值，以及用户在 PRODUCT.md 里提到的具名反模式。**PRODUCT.md 里的每条 anti-reference 都应当在这里以同样措辞出现在某条 "Don't" 中**，这样视觉规范才能把战略立场贯彻到底。能直接引用 PRODUCT.md 就直接引用：如果 PRODUCT.md 里写的是 *"avoid dark mode with purple gradients, neon accents, glassmorphism"*，这里的 Don'ts 也应当点名重复这些词。
...
```json
{
  ...
  "narrative": {
    ...
    "dos": ["Do use the primary accent sparingly...", "..."],
    "donts": ["Don't use border-left greater than 1px as a colored stripe", "..."]
  }
}
``````json
{
...
**相较 schemaVersion 1 的变化。** 旧版 sidecar 里有 token 基础数组（`tokens.colors[]`、`tokens.typography[]` 等）。现在这些值都放进 frontmatter 了。sidecar 只保留那些 frontmatter 无法承载的元数据——如色调阶梯、当 hex 只是近似值时对应的 canonical OKLCH、显示名称、角色提示——并且它们都通过 frontmatter 的 token 名来索引（如 `colorMeta.<token-name>`、`typographyMeta.<token-name>`）。组件仍然保留完整 HTML/CSS，因为 Stitch 那 8 个属性装不下这些内容。
...
1.  把完整的 DESIGN.md 展示给用户。简短点出其中那些不那么显而易见但很关键的创意决策（如描述性颜色命名、氛围语言、具名规则）。
2.  说明 `DESIGN.json` 也已经一并写好，live panel 现在会渲染这个项目真实的 button / input / nav primitives，而不是泛化近似物。
3.  提供微调选项：“要我修改某个章节、补上漏掉的组件模式，或调整氛围语言吗？”
...
开始访谈前先说明：“目前没有现成的视觉系统可供扫描。我会先问五个简短问题，生成一份起步用的 DESIGN.md。等你有代码后，可以重新运行 `/impeccable document`，捕获真实的 tokens 和组件。可以吗？”
...
3.  **动效能量。** 三选一：
    -   Restrained - 只处理状态变化
    -   Responsive - 有反馈和过渡，但没有编排式动态编排
    -   Choreographed - 有编排式入场和滚动驱动序列

4.  **三个具名参考。** 可以是品牌、产品、印刷物。不要给形容词。

5.  **一个 anti-reference。** 它绝对不应该是什么感觉。也必须是具名对象。

### 第 3 步：写 seed DESIGN.md

沿用扫描模式的六章节规范。把访谈里回答过的内容写进去，其他位置诚实地留成占位。seed 是脚手架，不是编造出来的完整规范。
...
Seed 模式下，frontmatter 只写最小字段：`name` 和 `description`，不要写 colors、typography、rounded、spacing 或 components。真正的 tokens 等下一次扫描模式再落地。出于同样原因，Seed 模式也跳过 `DESIGN.json` sidecar：此时没有可渲染的东西。

### 第 4 步：确认并刷新会话缓存

1.  把 seed DESIGN.md 展示给用户，并明确指出它是 seed（这个标记本身就是承诺）。
2.  告诉用户：“等你有了一些代码后，再运行 `/impeccable document`。那一轮会提取真实 tokens，并生成 sidecar。”
...
-   **描述性优先于技术性**：像“柔和圆角（8px radius）”这种写法比“rounded-lg”更好。技术值放在括号里，但前面先给感受描述。
...
-   **语气要强。** 要像设计总监说话。用“禁止”“绝不”“始终”“必须”，不要用“可以考虑”“也许”“更倾向于”。与 PRODUCT.md 保持同样的语气强度。
-   **使用具体反模式测试语句。** Stitch 会写出类似 *“如果它看起来像 2014 年的应用，那阴影就太深、模糊就太小了。”* 这样的句子。一句可审计测试语句，胜过一整段抽象原则。
-   **引用 PRODUCT.md。** 那里的 anti-references 应直接影响这里的 Do's and Don'ts。可以直接引用，也可以准确转述。
-   **按角色分组颜色**，而不是按 hex 顺序或色相顺序。规范顺序是主色 / 次色 / 第三色 / 中性色。
...
-   不要哪怕轻微地改章节名。必须是 “Colors”，不是 “Color Palette & Roles”；必须是 “Typography”，不是 “Typography Rules”。工具链解析依赖精确标题。
