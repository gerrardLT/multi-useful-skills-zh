...
**进度跟踪**：使用 LocalStorage 记录 “seen” 状态  
**分析**：跟踪完成率、流失点
...
**重要**：不要让用户重复看到同一套 onboarding（非常烦人）。一定要记录完成状态，并尊重关闭操作。
...
- 不要忘记回访用户（不要再次给他们看初始 onboarding）
