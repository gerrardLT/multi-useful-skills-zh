...
2. **识别瓶颈**：
   ...
   - 严重程度如何？（用户能否感知？是否令人烦躁？是否阻塞操作？）
   - 影响范围？（所有用户？仅移动端？仅慢网环境？）
...
**优化图片**：
- 使用现代格式（WebP、AVIF）
- 采用合适尺寸（避免为 300px 显示加载 3000px 图片）
- 对首屏以下图片使用懒加载
...
**优化加载策略**：
- 确保关键资源优先加载（非关键脚本使用 async / defer）
- 预加载关键资源
- 预取高概率访问的下一页
...
**减少绘制与合成压力**：
- 位移动画优先使用 `transform` 和 `opacity`，但当模糊、滤镜、遮罩、裁剪路径、阴影、颜色偏移能显著提升质感时，也可使用
- 避免随意动画化布局属性（`width`、`height`、`top`、`left`、margin）
- 仅在明确昂贵的操作上少量使用 `will-change`
- 对高开销的模糊/滤镜/阴影效果控制作用区域（越小、越隔离越快）
...
**重要提示**：务必在真实设备和真实网络环境下测试。桌面端 Chrome + 快速网络并不具有代表性。
...
- **不同设备测试**：在低端 Android 上测试，不要只在旗舰 iPhone 上测试
- **慢网环境测试**：限速至 3G 以检验体验
- **无功能回归**：确认功能未被优化破坏
- **用户感知**：是否真的“感觉更快了”？
