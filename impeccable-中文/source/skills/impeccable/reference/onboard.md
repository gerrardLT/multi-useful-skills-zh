...
**关键点**：Onboarding 的目标是让用户尽快到达价值，不是把所有东西都教完。
...
**空状态类型**：
- **首次使用**：从未使用过这个功能（强调价值，提供模板）
- **用户清空**：用户主动删空了所有内容（处理轻一点，方便重建）
- **无结果**：搜索 / 筛选后没有结果（建议换查询词、清除筛选）
- **无权限**：没有权限访问（解释原因，并说明如何获得权限）
- **错误状态**：加载失败（解释发生了什么，并提供重试）
...
**重要**：不要让用户重复看到同一套 onboarding（非常烦人）。一定要记录 completion，并尊重 dismiss。
...
- **完成时间**：用户能否快速完成 onboarding？
- **理解程度**：完成之后，他们真的理解了吗？
- **行动**：他们是否完成了我们期望的下一步动作？
- **跳过率**：是不是太多人直接跳过？（那可能说明它太长，或没价值）
- **完成率**：用户是否愿意走完？（如果太低，就简化）
- **价值获取时间**：用户要多久才能拿到第一次价值？
