...
2.  **识别瓶颈**：
    ...
    *   有多严重？（用户能感知到吗？会感到烦恼吗？会阻塞操作吗？）
    *   谁会受影响？（所有用户？仅移动端用户？仅慢速网络环境用户？）
...
**优化图片**：
*   使用现代格式（WebP、AVIF）
*   使用正确的尺寸（不要为了 300px 的显示区域加载 3000px 的图片）
*   对首屏以下的图片使用懒加载
*   使用响应式图片（`srcset`、`picture`）
*   压缩图片（80-85% 的质量通常肉眼无法察觉差异）
*   使用 CDN 提升分发速度
...
**减少绘制与合成压力**：
*   位移动画优先使用 `transform` 和 `opacity`，但当模糊、滤镜、遮罩、剪切路径、阴影、颜色偏移能显著提升质感时，也可以使用
...
**重要**：要在真实设备和真实网络环境下测试。桌面 Chrome + 快速网络并不具有代表性。
...
记住：性能本身就是功能。更快的体验会让产品显得更灵敏、更精致、更专业。系统性地进行优化，严格测量，并优先处理用户能真实感知到的性能问题。