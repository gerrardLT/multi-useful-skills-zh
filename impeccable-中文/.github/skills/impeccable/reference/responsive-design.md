...
首先编写移动端基础样式，再通过 `min-width` 查询逐层叠加复杂度。桌面优先（`max-width`）意味着移动端会先加载一堆自己根本不需要的样式。

## 断点：由内容驱动

不要追着设备尺寸跑——让内容决定断点位置。先从窄屏开始，逐步拉宽，等设计开始坏掉的那个点就是断点。通常 3 个断点就够了（640、768、1024px）。配合 `clamp()`，很多值甚至可以不靠断点实现流动变化。
