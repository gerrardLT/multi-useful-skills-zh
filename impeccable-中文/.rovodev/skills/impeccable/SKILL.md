---
name: impeccable
description: 当用户想要设计、重设计、shape、critique、audit、polish、clarify、distill、harden、optimize、adapt、animate、colorize、extract，或以其他方式改进前端界面时使用。覆盖网站、落地页、仪表盘、产品 UI、应用外壳、组件、表单、设置页、onboarding 和空状态。可处理 UX review、视觉层级、信息架构、认知负荷、可访问性、性能、响应式行为、主题、反模式、排版、字体、间距、布局、对齐、颜色、动效、微交互、UX 文案、错误状态、边界情况、i18n，以及可复用的设计系统或 tokens。也适用于那些需要变得更大胆、更有趣的平淡设计，需要变得更安静的吵闹设计，对浏览器中 UI 元素进行实时迭代，或追求技术上足够惊艳的视觉效果。不适用于纯后端或非 UI 任务。
---
...
| 关卡 | 必要检查 | 如果失败 |
|---|---|---|
| 上下文 | 已通过 `node .agents/skills/impeccable/scripts/load-context.mjs` 获取到 PRODUCT.md / DESIGN.md 的加载结果。 | 先运行加载器再继续。 |
| 产品 | PRODUCT.md 存在，并且不是空文件或占位内容（如 `[TODO]` 标记，或少于 200 字符）。 | 运行 `$impeccable teach`，刷新上下文后再继续。绝不能只根据用户原始提示词自行编造 PRODUCT.md。 |
| 命令 | 使用子命令时，已经加载对应的命令参考。 | 先加载参考再继续。 |
| 工艺 | 对当前任务来说，`$impeccable craft` 已拥有用户确认过的 shape brief。`teach` / PRODUCT.md 永远不算 shape。 | 运行 `$impeccable shape` 并等待用户明确确认 brief。 |
| 图像 | 必需的视觉探针 / mock 已生成，或已说明跳过理由。 | 在写代码前，先解决 `shape.md` 或 `craft.md` 里的图像生成关卡。 |
| 变更 | 以上所有活动关卡都已通过。 | 现在还不要编辑项目文件。 |
...
- **PRODUCT.md**：必需。包含用户、品牌、语气、anti-references、战略原则。
...
如果 DESIGN.md 缺失：每个会话只提醒一次（*"运行 `$impeccable document` 以获得更符合品牌规范的输出"*），然后继续。

### 2. 注册

每个设计任务都属于 **品牌**（营销、落地页、活动页、长内容、作品集，设计本身就是产品）或 **产品**（应用 UI、后台、仪表盘、工具，设计服务于产品）之一。
...
加载对应的参考：[reference/brand.md](reference/brand.md) 或 [reference/product.md](reference/product.md)。下面的共享设计法则对两者都适用。
...
- 在选颜色之前，先确定 **颜色策略**。它沿着承诺度轴分成四档：
  - **克制型**：染色中性色 + 一个强调色，占比不超过约 10%。是产品的默认策略，也是品牌极简风格的默认策略。
  - **承诺型**：一个高饱和色承担表面 30–40% 的面积。是强品牌识别页面的默认策略。
  - **全调色板**：有 3–5 个命名角色色，并且都被有意使用。适用于品牌活动页和产品中的数据可视化。
  - **浸染型**：表面本身就是颜色。适用于品牌 hero 和活动页面。
- “一个强调色不超过 10%” 只适用于克制型。承诺型 / 全调色板 / 浸染型本来就会主动超过这个比例。不要条件反射地把所有设计都压回克制型。
...
如果有人看一眼这个界面，就能毫无疑问地说出“这就是 AI 做的”，那它就失败了。跨 register 的失败信号就是上面的那些绝对禁令。各自 register 的特定失败模式写在各自的参考里。
...
| `extract [target]` | 构建 | 将可复用 tokens 和组件提取进设计系统 | [reference/extract.md](reference/extract.md) |
...
| `bolder [target]` | 优化 | 放大过于安全或平淡的设计表达 | [reference/bolder.md](reference/bolder.md) |
| `quieter [target]` | 优化 | 压低过于激进或过度刺激的设计 | [reference/quieter.md](reference/quieter.md) |
...
| `colorize [target]` | 增强 | 为单色 UI 加入有策略的色彩 | [reference/colorize.md](reference/colorize.md) |
...
| `clarify [target]` | 修复 | 改善 UX 文案、标签和错误消息 | [reference/clarify.md](reference/clarify.md) |
...
2. **第一个词匹配某个 command**：加载对应的参考文件并遵循其中指令。command 名后的所有内容都视为 target。
3. **第一个词不匹配任何 command**：作为一般设计调用处理。应用设置步骤、共享设计法则和已加载的 register 参考，并把完整参数作为上下文。

到这一步为止，设置（上下文收集、register 识别）应当已经完成；子命令不应再次调用 `$impeccable`。
