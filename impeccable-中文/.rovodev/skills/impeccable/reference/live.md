# 实时模式（Live Variant）：在浏览器中选择元素，挑一个设计动作，然后通过 dev server 的 HMR 热替换 AI 生成的 HTML + CSS 变体。
...
这三种错误都会带上 `fallback: "agent-driven"`。继续看下文 **处理 fallback**。
...
```wrap script 已经为你提供了一个单层 JSX wrapper，也就是一个外层 `<div data-impeccable-variants="...">`，marker comments 也已放好。只需要把上面的 variants block 填进 “Variants: insert below this line” 那个 comment 下面，源文件就仍然是合法 TSX。
...
- **Leaf / tiny**：单个按钮、图标、输入框、独立标题、单段文字：**0 个 params**。
