...
**placeholder 不是 label**，因为一输入就消失。始终使用可见的 `<label>`。**应在失焦时校验**，而不是每次敲键都校验（密码强度是例外）。错误信息应放在字段**下方**，并用 `aria-describedby` 关联起来。

## 加载状态

**乐观更新**：先立即展示成功，失败时再回滚。适用于低风险操作（点赞、关注），不适用于支付或破坏性行为。**骨架屏优于 spinner**，因为它能提前展示内容轮廓，也比通用 spinner 更让人感觉快。
...
**优点**：支持轻量级关闭（点外部即可关闭）、层叠顺序正确、不用打 `z-index` 混战，而且默认更易访问。
