运行系统化的 **技术性** 质量检查，并生成一份完整报告。不要在此处修复问题——记录问题，交由其他命令处理。
...
**评级区间**：18-20 Excellent（只需少量 polish），14-17 Good（修补薄弱维度），10-13 Acceptable（仍需显著工作），6-9 Poor（需要大幅重做），0-5 Critical（存在根本性问题）
...
- **[P?] 问题名称**
- **位置**：组件、文件、行号
- **类别**：Accessibility / Performance / Theming / Responsive / Anti-Pattern
- **影响**：它如何影响用户
...
- “整个移动端体验中 touch targets 持续过小 (<44px)”
