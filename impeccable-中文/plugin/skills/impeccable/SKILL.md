...
| 关卡 | 必要检查 | 如果失败 |
|---|---|---|
| 上下文 | 已通过 `node .agents/skills/impeccable/scripts/load-context.mjs` 获取到 PRODUCT.md / DESIGN.md 的加载结果。 | 先运行 loader 再继续。 |
| 产品 | PRODUCT.md 存在，并且不是空文件或占位内容（如 `[TODO]` 标记，或少于 200 字符）。 | 运行 `$impeccable teach`，刷新上下文后再继续。绝不能只根据用户原始提示词自行编造 PRODUCT.md。 |
| 命令 | 使用子命令时，已经加载对应的 command reference。 | 先加载 reference 再继续。 |
...
- 在选颜色之前，先确定 **颜色策略**。它沿着承诺度轴分成四档：
  - **克制型**：染色中性色 + 一个强调色，占比不超过约 10%。是 product 的默认策略，也是 brand 极简风格的默认策略。
  - **承诺型**：一个高饱和色承担表面 30–40% 的面积。是强品牌识别页面的默认策略。
  - **全色板型**：有 3–5 个命名角色色，并且都被有意使用。适用于品牌活动页和产品中的数据可视化。
  - **浸染型**：表面本身就是颜色。适用于品牌 hero 和活动页面。
- “一个强调色不超过 10%” 只适用于克制型。承诺型 / 全色板型 / 浸染型 本来就会主动超过这个比例。不要条件反射地把所有设计都压回克制型。
...
**固定** 会创建一个独立快捷入口，让 `$<command>` 直接调用 `$impeccable <command>`。**取消固定** 会移除它。这个脚本会向项目中所有已存在的 harness 目录写入对应内容。
