...
2. **识别瓶颈**：
   - 具体是什么慢？（首屏加载？交互响应？动画？）
   - 慢的原因是什么？（大图片？高开销 JavaScript？布局抖动？）
   - 严重程度如何？（用户能感知到吗？会感到烦躁吗？会阻塞操作吗？）
   - 影响范围？（所有用户？仅移动端用户？仅慢网络环境用户？）

**关键原则**：先测量，再优化。过早优化只会浪费时间。只优化真正重要的地方。
...
**优化图片**：
- 使用现代格式（WebP、AVIF）
- 使用正确尺寸（不要为 300px 的显示区域加载 3000px 的图片）
- 首屏以下的图片使用懒加载
- 使用响应式图片（`srcset`、`picture`）
- 压缩图片（80-85% 的质量通常肉眼难以察觉差异）
- 使用 CDN 提升分发速度
...
**减少绘制与合成压力**：
- 位移动画优先使用 `transform` 和 `opacity`，但当模糊、滤镜、遮罩、裁剪路径、阴影、颜色偏移能显著提升质感时，也可以使用
- 避免随意动画化布局属性（`width`、`height`、`top`、`left`、margin）
- 仅在明确昂贵的操作上少量使用 `will-change`
- 对高开销的模糊 / 滤镜 / 阴影效果控制其作用区域（越小、越隔离越快）
...
**重要提示**：必须在真实设备和真实网络环境下测试。桌面端 Chrome + 快速网络不具有代表性。

**切勿**：
- 不测量就开始优化（过早优化）
- 为了性能牺牲可访问性
- 优化到破坏功能
- 到处滥用 `will-change`（会创建新图层，占用内存）
- 懒加载首屏内容
- 纠结于微优化，却放过大瓶颈（永远先优化最重的瓶颈）
- 忽略移动端性能（通常设备更慢、网络更差）
...
- **不同设备**：在低端 Android 设备上测试，不要只在旗舰 iPhone 上测试
- **慢网络环境**：限速到 3G 查看体验
- **无功能回归**：确认功能没有被优化破坏
- **用户感知**：它是否真的“感觉更快了”？

记住：性能本身就是功能。更快速的体验会让产品显得更灵敏、更精致、更专业。进行系统性优化，严格测量，并优先处理用户能真实感知到的性能问题。