...
**最常见的遗漏**：只设计悬停状态而不设计聚焦状态，或者反过来。两者不是一回事。键盘用户永远无法看到悬停状态。
...
**优点**：支持轻量级关闭（点击外部即可关闭）、层叠关系正确、无需处理 `z-index` 冲突，而且默认更易访问。

## 下拉菜单与浮层定位

如果下拉菜单使用 `position: absolute` 渲染在带有 `overflow: hidden` 或 `overflow: auto` 的容器里，它就会被裁切。这是生成式代码中最常见的下拉菜单问题。

### CSS 锚点定位

现代方案是使用 CSS 锚点定位 API，在不依赖 JavaScript 的情况下将浮层锚定到触发器：
...
由于下拉菜单使用的是 `position: fixed`，它可以逃离祖先元素的 `overflow` 裁切。`@position-try` 块还能自动处理视口边界。**浏览器支持**：Chrome 125+、Edge 125+。Firefox 和 Safari 尚不支持，因此这些浏览器需要兜底方案。
...
`popover` 属性会将元素放入 **顶层**，无论 `z-index` 或 `overflow` 如何，都会覆盖在普通内容之上，因此不需要 portal。

### Portal / Teleport 模式

在组件框架中，可以将下拉菜单渲染到文档根节点，再用 JavaScript 定位：
...
- **将下拉菜单直接内联渲染**，却没有办法逃离父级层叠上下文。应使用 `popover`（顶层）、portal，或 `position: fixed`。
...
为键盘用户提供跳过链接（`<a href="#main-content">跳过导航，直接访问主要内容</a>`），让他们可以跳过导航直接进入正文。默认可移到屏幕外，聚焦时再显示。
...
**避免**：在没有替代方案时移除焦点指示；将占位符当作标签；触控目标小于 44x44px；使用泛泛的错误信息；制作没有 ARIA / 键盘支持的自定义控件。