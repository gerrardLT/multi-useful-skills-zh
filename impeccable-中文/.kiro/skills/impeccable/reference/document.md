在项目根目录生成一个 `DESIGN.md` 文件，捕获当前视觉设计系统，以便 AI agent 在生成新界面时能保持品牌一致性。
...
**相较于 schemaVersion 1 的变化。** 旧版 sidecar 中包含 token 基础数组（`tokens.colors[]`、`tokens.typography[]` 等）。现在这些值都放入了 frontmatter。sidecar 仅保留那些 frontmatter 无法承载的元数据——如 tonal ramps、当 hex 只是近似值时对应的 canonical OKLCH、显示名称、角色提示——并且它们都通过 frontmatter 的 token 名称来索引（如 `colorMeta.<token-name>`、`typographyMeta.<token-name>`）。组件仍然保留完整的 HTML/CSS，因为 Stitch 的 8 个属性装不下这些内容。
