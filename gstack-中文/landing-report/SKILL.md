...
version: 0.1.0
description: |-
  用于工作区感知的只读版本队列仪表板。显示当前哪个版本槽位
  已被开放的 PR 认领，兄弟 Conductor 工作区已拥有哪些
  WIP 工作可能很快就会发布，以及接下来会选择哪个槽位/发布。无
  突变——仅提供快照。当被要求“着陆报告”、“队列里有什么”、
  “向我展示开放的 PR 版本”或“我接下来该认领哪个版本”时使用。 （gstack）
triggers:
- landing report
- version queue
- ship queue
...
当您运行 5-10 个并行 Conductor 工作区时，它有助于查看 —
一目了然——哪些版本号被声明，由谁声明，以及您的下一个版本号是什么
`/ship` 将登陆。该技能是对同一技能的只读调用
`bin/gstack-next-version` 实用程序 `/ship` 使用，但没有任何变化。
将其视为版本号的 `gh pr list` 。
...
调用 util 三次 - 每个凹凸级别调用一次 - 这样用户就可以看到什么
他们会声称 micro/patch/minor/major。便宜（bun 缓存的相同 gh 调用）。
...
构建单个表输出。使用 `patch` 级别的 JSON 作为规范
队列 + 兄弟姐妹（它们在凹凸级别上是相同的；只有 `.version`
不同）。

使用 `jq` 提取：
- `.host` — github|GitLab|未知
- `.offline` — 查询失败了吗？
- `.claimed` — {pr、分支、版本、url} 数组
- `.siblings` — 找到所有同级工作树
- `.active_siblings` — 可能即将发货的子集
...
同级 Conductor 工作树 (<workspace_root>)：
  路径                        分支                 版本         最后提交      PR
  ──────────────────────────────────────────────────────────────────────────────────
  ../tokyo-v2                 feat/dashboard         v1.7.1.0    3小时前        无  ★ 活跃
  ../melbourne                feat/review            v1.6.0.0    12天前        无
  ../osaka                    feat/payments          v1.8.0.0    5小时前        #1155

★ 活跃 = 版本领先于基准 且 最后提交在 24 小时内 且 无开放 PR。
  这些是可能很快发布的版本。

如果你现在运行 /ship，你将声明：
  微版本:  v1.6.3.1   (队列推进: 无)
  补丁版本:  v1.7.1.0   (已提升超过已声明的 1.7.0.0)
  次要版本:  v1.8.0.0   (已提升超过已声明的 1.7.0.0)
  主要版本:  v2.0.0.0   (无主要冲突)
```
...
1.  **如果队列中存在冲突**（两个开放的 PR 声明相同的版本）：
“⚠ 两个开放的 PR 在 v<X> 上发生冲突。第二个合并的人将覆盖
第一个的变更日志条目或复制一个。考虑询问一位作者
重新运行 /ship 以获取下一个空闲插槽。”

2.  **如果活跃兄弟版本的排名超过用户的分支版本：**
“同级工作树 <path> 已在 <N> 小时前提交了 v<X>，但尚未 PR。
如果该工作首先发货，您的分支将需要在着陆时重新启动。”

3.  **如果一切看起来都很干净：**
“队列是干净的。接下来 /ship 将声明一个槽位，不会发生冲突。”

---

## 计划模式

计划模式例外 - 始终运行。该技能完全只读：无文件
写入，无 git 变更，无网络状态变化。在计划模式下安全运行。