...
// Dynamic template discovery 鈥?matches the generator's findTemplates() behavior.
...
describe('gen-skill-docs', () => {
  ...
  test('tier 1 skills do NOT contain AskUserQuestion format', () => {
    // Use benchmark (tier 1) instead of root 鈥?root SKILL.md gets overwritten by Codex test setup
  ...
  test('plan-review generated preambles stay under the Option A budget', () => {
    ...
    // Plan skills carry the same preamble surface as other tier-鈮? skills
    ...
    for (const skill of CLAUDE_GENERATED_SKILLS) {
      ...
      for (const block of bashBlocks) {
        ...
        for (const line of lines) {
          ...
          // Check 1: "for VAR in <glob>" must use $(find ...) 鈥?caught above by the
  ...
  test('qa-only has no-fix guardrails', () => {
    ...
    // Scope to frontmatter (between the first two --- lines) 鈥?the body can
...
/**
 * Quality evals 鈥?catch description regressions.
 *
...
describe('description quality evals', () => {
  ...
  // Guard: generated output uses 鈫?not ->
  test('generated SKILL.md uses unicode arrows', () => {
    const content = fs.readFileSync(path.join(ROOT, 'SKILL.md'), 'utf-8');
    // Check the Tips section specifically (where we regressed -> from 鈫?
    const tipsSection = content.slice(content.indexOf('## Tips'));
    expect(tipsSection).toContain('鈫?);
...
// 鈹€鈹€鈹€ Test Coverage Audit Resolver Tests 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
describe('TEST_COVERAGE_AUDIT placeholders', () => {
  ...
  test('plan and ship modes include E2E decision matrix', () => {
    ...
    for (const skill of [planSkill, shipSkill]) {
      ...
      expect(skill).toContain('鈫扙2E');
      expect(skill).toContain('鈫扙VAL');
    }
  });
  ...
  test('ship SKILL.md regression guard 鈥?key phrases preserved', () => {
...
describe('PLAN_COMPLETION_AUDIT placeholders', () => {
  ...
  test('ship mode has gate logic for NOT DONE items', () => {
    ...
    expect(shipSkill).toContain('Stop 鈥?implement the missing items');
    expect(shipSkill).toContain('Ship anyway 鈥?defer');
...
describe('Coverage gate in ship', () => {
  ...
  test('coverage gate skips on parse failure (not block)', () => {
    expect(shipSkill).toContain('could not determine percentage 鈥?skipping');
  });
...
describe('CODEX_SECOND_OPINION resolver', () => {
  ...
  test('Phase 3.5 section appears in office-hours SKILL.md', () => {
    expect(content).toContain('Phase 3.5');
  });
  ...
  test('contains cross-model synthesis instructions', () => {
    expect(content).toMatch(/[Ss]ynthesis|综合/);
    expect(content).toContain('Claude 与第二意见一致的地方');
  });

  test('contains Claude subagent fallback', () => {
    expect(content).toContain('CODEX_NOT_AVAILABLE');
    expect(content).toContain('Agent tool');
    expect(content).toContain('第二意见（Claude 子代理）');
  });
...
describe('Codex filesystem boundary', () => {
  ...
  const CODEX_CALLING_SKILLS = [
    'codex',         // /codex skill 鈥?3 modes
    'autoplan',      // /autoplan 鈥?CEO/design/eng voices
    'review',        // /review 鈥?adversarial step resolver
    'ship',          // /ship 鈥?adversarial step resolver
    'plan-eng-review',  // 澶栭儴瑙嗚瑙ｆ瀽鍣?    'plan-ceo-review',  // 澶栭儴瑙嗚瑙ｆ瀽鍣?    'office-hours',     // 绗簩鎰忚瑙ｆ瀽鍣?  ];
...
describe('BENEFITS_FROM resolver', () => {
  ...
  test('inline invocation 鈥?no "another window" language', () => {
    expect(ceoContent).not.toContain('another window');
    expect(engContent).not.toContain('another window');
  });

  test('inline invocation 鈥?read-and-follow path present', () => {
...
describe('DESIGN_SKETCH 已扩展外部视角', () => {
  const content = fs.readFileSync(path.join(ROOT, 'office-hours', 'SKILL.md'), 'utf-8');
  ...
  test('offers opt-in via AskUserQuestion', () => {
    expect(content).toContain('外部设计视角');
  });
...
// 鈹€鈹€鈹€ Codex Generation Tests 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€

describe('Codex generation (--host codex)', () => {
  const AGENTS_DIR = path.join(ROOT, '.agents', 'skills');

  // .agents/ is gitignored (v0.11.2.0) 鈥?generate on demand for tests
  ...
  // (vendored dev mode 鈥?gen-skill-docs skips these to avoid overwriting Claude SKILL.md)
  const CODEX_SKILLS = (() => {
  ...
  test('multiline descriptions preserved in Codex output', () => {
    // office-hours has a multiline description 鈥?verify it survives the frontmatter transform
  ...
  // 鈹€鈹€鈹€ Path rewriting regression tests 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€

  test('sidecar paths point to .agents/skills/gstack/review/ (not gstack-review/)', () => {
    // Regression: gen-skill-docs rewrote .claude/skills/review 鈫?.agents/skills/gstack-review
  ...
  test('all four path rewrite rules produce correct output', () => {
    ...
    // Rule 1: ~/.claude/skills/gstack 鈫?$GSTACK_ROOT
    expect(content).not.toContain('~/.claude/skills/gstack');
    expect(content).toContain('$GSTACK_ROOT');

    // Rule 2: .claude/skills/gstack 鈫?.agents/skills/gstack
    expect(content).not.toContain('.claude/skills/gstack');

    // Rule 3: .claude/skills/review 鈫?.agents/skills/gstack/review
    expect(content).not.toContain('.claude/skills/review');

    // Rule 4: .claude/skills 鈫?.agents/skills (catch-all)
    expect(content).not.toContain('.claude/skills');
  });
  ...
  // 鈹€鈹€鈹€ Claude output regression guard 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
  ...
  test('Claude output unchanged: all Claude skills have zero Codex paths', () => {
    for (const skill of CLAUDE_GENERATED_SKILLS) {
      ...
      // and log path (~/.codex/logs/) 鈥?those are user-facing Codex CLI paths,
  ...
  // 鈹€鈹€鈹€ 璁捐澶栭儴瑙嗚锛欳odex host 瀹堝崼 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€

  test('codex host 在 design-review 中不会产出设计外部视角', () => {
    const codexContent = fs.readFileSync(path.join(AGENTS_DIR, 'gstack-design-review', 'SKILL.md'), 'utf-8');
    expect(codexContent).not.toContain('设计外部视角');
  });
...
// 鈹€鈹€鈹€ Factory generation tests 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
describe('Factory generation (--host factory)', () => {
  ...
  test('Factory keeps Codex integration blocks', () => {
    // Factory 鐢ㄦ埛涔熷彲浠ヤ娇鐢?Codex 绗簩鎰忚锛坈odex exec 鏄嫭绔嬩簩杩涘埗锛?    const shipContent = fs.readFileSync(path.join(FACTORY_DIR, 'gstack-ship', 'SKILL.md'), 'utf-8');
...
// 鈹€鈹€鈹€ Parameterized host smoke tests (config-driven) 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
// 鈹€鈹€鈹€ --host all tests 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
// 鈹€鈹€鈹€ Setup script validation 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
// These tests verify the setup script's install layout matches
// what the generator produces 鈥?catching the bug where setup
...
describe('setup script validation', () => {
  ...
  // T1: Sidecar skip guard 鈥?prevents .agents/skills/gstack from being linked as a skill
...
describe('community fixes wave', () => {
  ...
  // #594 鈥?Discoverability: every SKILL.md.tmpl description contains "gstack"
  ...
  // #594 鈥?Discoverability: first line of each description is under 120 chars
  ...
  // #573 鈥?Feature signals: ship/SKILL.md contains feature signal detection
  test('ship/SKILL.md contains feature signal detection in Step 4', () => {
    const content = fs.readFileSync(path.join(ROOT, 'ship', 'SKILL.md'), 'utf-8');
    expect(content.toLowerCase()).toContain('feature signal');
  });

  // #510 鈥?Context warnings: no SKILL.md contains "running low on context"
  ...
  // #510 鈥?Context warnings: plan-eng-review has explicit anti-warning
  test('plan-eng-review/SKILL.md contains "Do not preemptively warn"', () => {
    const content = fs.readFileSync(path.join(ROOT, 'plan-eng-review', 'SKILL.md'), 'utf-8');
    expect(content).toContain('Do not preemptively warn');
  });

  // #474 鈥?Safety Net: no SKILL.md uses find with -delete
  ...
  // #467 鈥?Telemetry: preamble JSONL writes are gated by telemetry setting
    ...
    for (const rel of allFiles) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (line.includes('codex review') && line.includes('$(git rev-parse --show-toplevel)')) {
          violations.push(`${rel}:${i + 1} 鈥?inline git rev-parse in codex review`);
        }
      }
    }
    expect(violations).toEqual([]);
...
// 鈹€鈹€鈹€ Learnings + Confidence Resolver Tests 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
describe('plan-mode-info resolver (handshake-replacement)', () => {
  ...
  test('vestigial handshake is absent from non-Claude host outputs when present on disk', () => {
    ...
    // plan-mode-info resolver has no host-scoping 鈥?all hosts get the new
    ...
    if (checked === 0) {
      ...
      console.warn(
        'plan-mode-info: no non-Claude host outputs found for cross-host absence check 鈥?' +
