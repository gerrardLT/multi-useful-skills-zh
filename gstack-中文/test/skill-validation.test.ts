...
describe('Usage string consistency', () => {
  ...
  function skeleton(usage: string): string {
    return usage
      ...
      .replace(/<[^>]*>/g, '<>')      // normalize <param-name> 鈫?<>
      .replace(/\[[^\]]*\]/g, '[]')   // normalize [optional] 鈫?[]
...
describe('No hardcoded branch names in SKILL templates', () => {
  ...
  const allowlist = [
    ...
    /fall\s*back\s+to\s+`?main`?/i,
    /typically\s+`?main`?/i,
    /If\s+on\s+`main`/i,  // old pattern 鈥?should not exist
  ];
...
describe('v0.4.1 preamble features', () => {
  ...
  for (const skill of tier2PlusSkills) {
    test(`${skill} contains AskUserQuestion Pros/Cons format`, () => {
      ...
      expect(content).toMatch(/鉁?);
      expect(content).toMatch(/鉂?);
    });
  }
...
// Contributor mode was removed in v0.13.10.0 鈥?replaced by operational self-improvement.
...
describe('Enum & Value Completeness in review checklist', () => {
  ...
  test('Enum & Value Completeness is classified as CRITICAL', () => {
    // It should appear under Pass 1 鈥?CRITICAL, not Pass 2
  ...
  test('Enum & Value Completeness is in the severity classification as CRITICAL', () => {
    ...
    // Enum & Value Completeness should appear on a line with the CRITICAL tree (鈹溾攢 or 鈹斺攢)
    const enumLine = gateSection.split('\n').find(l => l.includes('Enum & Value Completeness'));
    expect(enumLine).toBeDefined();
    // It's on the left (CRITICAL) side 鈥?starts with 鈹溾攢 or 鈹斺攢
    expect(enumLine!.trimStart().startsWith('鈹溾攢') || enumLine!.trimStart().startsWith('鈹斺攢')).toBe(true);
  });
...
describe('gstack-slug', () => {
  ...
  test('no templates or bin scripts use source process substitution for gstack-slug', () => {
    ...
    // grep returns exit code 1 when no matches found 鈥?that's what we want
...
describe('Step 3.4 test coverage audit', () => {
  ...
  test('Step 3.4 includes quality scoring rubric', () => {
    const content = fs.readFileSync(path.join(ROOT, 'ship', 'SKILL.md'), 'utf-8');
    expect(content).toContain('鈽呪槄鈽?);
    expect(content).toContain('鈽呪槄');
...
describe('ship step numbering', () => {
  ...
  // 9.3 (Cross-review dedup), 15.0 (WIP squash 鈥?continuous checkpoint), 15.1 (Bisectable commits).
  ...
  test('ship/SKILL.md main headings use clean integer step numbers', () => {
    ...
    // Headings like "## Step 7: Test Coverage Audit" 鈥?NOT sub-steps like "## Step 8.1:"
...
describe('Codex skill', () => {
  ...
  test('codex-host ship/review do NOT contain adversarial review step', () => {
    // .agents/ is gitignored 鈥?generate on demand
...
describe('Skill trigger phrases', () => {
  // 杩欎簺鎶€鑳界殑 description 閲屽繀椤讳繚鐣?鈥淯se when鈥?  // Excluded: root gstack (browser tool), gstack-upgrade (gstack-specific),
  // humanizer (text tool)
  ...
  for (const skill of SKILLS_REQUIRING_TRIGGERS) {
    test(`${skill}/SKILL.md 鍖呭惈 鈥淯se when鈥?鐭`, () => {
      ...
      expect(frontmatter).toMatch(new RegExp(['Use', 'when'].join(' '), 'i'));
    });
  }
...
// 鈹€鈹€鈹€ Codex Skill Validation 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
...
describe('Codex skill validation', () => {
  ...
  test('/codex skill is Claude-only 鈥?no Codex variant', () => {
  ...
  test('/claude skill is external-host-only 鈥?no Claude-host variant', () => {
