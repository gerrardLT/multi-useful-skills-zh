...
version: 1.0.0
description: |-
  配置 /land-and-deploy 的部署设置。检测您的部署
  平台（Fly.io、Render、Vercel、Netlify、Heroku、GitHub Actions、自定义）、
  生产 URL、运行状况检查端点和部署状态命令。写
  CLAUDE.md 的配置，以便所有未来的部署都是自动的。
  在以下情况下使用：“设置部署”、“配置部署”、“设置登陆并部署”、
  “如何使用 gstack 进行部署”、“添加部署配置”。
triggers:
- configure deploy
- setup deployment
- set deploy platform
...
您正在帮助用户配置其部署，以便 `/land-and-deploy` 正常工作
自动地。您的工作是检测部署平台、生产 URL、运行状况
检查并部署状态命令 - 然后将所有内容保存到 CLAUDE.md。
...
- **建议：** 如果您的设置发生更改，请选择 A 进行更新。
- A) 从头开始​​重新配置（覆盖现有）
- B）编辑特定字段（显示当前配置，让我更改一件事）
- C) 完成 — 配置看起来正确
...
根据检测到的内容，指导用户完成特定于平台的配置。

#### Fly.io

如果检测到 `fly.toml`：

1. 提取应用程序名称：`grep -m1 "^app" fly.toml | sed 's/app = "\(.*\)"/\1/'`
2. 检查是否安装了 `fly` CLI：`which fly 2>/dev/null`
3. 如果已安装，请验证：`fly status --app {app} 2>/dev/null`
4. 推断 URL：`https://{app}.fly.dev`
5. 设置部署状态命令：`fly status --app {app}`
6. 设置运行状况检查：`https://{app}.fly.dev`（或 `/health`，如果应用程序有）

要求用户确认生产 URL。一些 Fly 应用程序使用自定义域。
...
2. 检查渲染 API 密钥：`echo $RENDER_API_KEY | head -c 4`（不要暴露完整密钥）
3. 推断 URL：`https://{service-name}.onrender.com`
4. 渲染会在推送到连接的分支时自动部署 - 无需部署工作流程
5. 设置健康检查：推断的 URL

请用户确认。渲染使用来自连接的 git 分支的自动部署 — 之后
合并到 main，渲染器会自动选择它。 /land-and-deploy 中的“部署等待”
应轮询渲染 URL，直到它响应新版本。
...
3. Vercel 在推送时自动部署 — 在 PR 上预览，在合并到主版本时进行生产
4. 设置运行状况检查：vercel 项目设置中的生产 URL
...
1.  **如何触发部署？**
    -   A) 自动推送到主程序（Fly、Render、Vercel、Netlify 等）
    -   B) 通过 GitHub Actions 工作流程
    -   C) 通过部署脚本或 CLI 命令（描述一下）
    -   D) 手动（SSH、仪表板等）
    -   E) 该项目未部署（库、CLI、工具）

2.  **生产 URL 是什么？**（自由文本 — 应用程序运行的 URL）

3.  **gstack 如何检查部署是否成功？**
    -   A) 对特定 URL 进行 HTTP 运行状况检查（例如 /health、/api/status）
    -   B) CLI 命令（例如 `fly status`、`kubectl rollout status`）
    -   C) 检查 GitHub Actions 工作流程状态
    -   D) 无自动化方式 — 只需检查 URL 加载

4.  **有任何合并前或合并后的钩子吗？**
    -   合并之前运行的命令（例如，`bun run build`）
    -   在合并之后但部署验证之前运行的命令

### 第四步：编写配置

阅读 CLAUDE.md（或创建它）。查找并替换 `## Deploy Configuration` 部分
如果存在，或者将其附加到末尾。
...
- **切勿泄露秘密。** 不要打印完整的 API 密钥、令牌或密码。
- **与用户确认。** 始终显示检测到的配置并在写入之前要求确认。
- **CLAUDE.md 是事实来源。** 所有配置都在那里 - 而不是在单独的配置文件中。
- **幂等。** 多次运行 /setup-deploy 会干净地覆盖以前的配置。
- **平台 CLI 是可选的。** 如果未安装 `fly` 或 `vercel` CLI，则回退到基于 URL 的运行状况检查。