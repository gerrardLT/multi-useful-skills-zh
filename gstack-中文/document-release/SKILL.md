...
version: 1.0.0
description: |-
  发布后文档更新。阅读所有项目文档，交叉引用
  差异，更新 README/ARCHITECTURE/CONTRIBUTING/CLAUDE.md 以匹配已发布的内容，
  完善 CHANGELOG 语气，清理 TODOS，并可选择修改版本。使用时
  要求“更新文档”、“同步文档”或“发布后文档”。
  在 PR 合并或代码发布后主动提出建议。 （gstack）
allowed-tools:
- Bash
- Read
- Write
...
您正在运行 `/document-release` 工作流程。这在 `/ship`** 之后运行（代码已提交，PR
存在或即将存在）但**在 PR 合并之前**。您的工作：确保每个文档文件
该项目中的内容是准确的、最新的，并且以友好的、用户转发的声音编写。

你基本上是自动化的。直接进行明显的事实更新。停下来只询问有风险的或
主观决定。

**仅停留：**
- Risky/questionable 文档更改（叙述、哲学、安全性、删除、大量重写）
- 版本碰撞决定（如果尚未碰撞）
- 添加新的 TODOS 项目
- 叙述性（而非事实）的跨文档矛盾
...
- 更新路径、计数、版本号
- 修复过时的交叉引用
- CHANGELOG 语音润色（细微的措辞调整）
- 标记 TODOS 完成
- 跨文档事实不一致（例如版本号不匹配）

**永远不要这样做：**
- 覆盖、替换或重新生成变更日志条目 - 仅修饰措辞，保留所有内容
- 无需询问即可升级版本 - 始终使用 AskUserQuestion 进行版本更改
- 在 CHANGELOG.md 上使用 `Write` 工具 — 始终使用 `Edit` 与精确的 `old_string` 匹配

---

## 第 1 步：飞行前和差异分析

1. 检查当前分支。如果在基础分支上，**中止**：“您在基础分支上。从功能分支运行。”
...
4. 将更改分类为与文档相关的类别：
- **新功能** — 新文件、新命令、新技能、新功能
- **行为改变** — 修改服务、更新 API、配置更改
- **删除的功能** — 删除的文件，删除的命令
- **基础设施** — 构建系统、测试基础设施、CI

5. 输出一个简短的摘要：“正在分析 M 次提交中更改的 N 个文件。找到了 K 个要审查的文档文件。”

---

## 第 2 步：每个文件的文档审核

阅读每个文档文件并将其与差异进行交叉引用。使用这些通用启发法
（适应您所在的任何项目 - 这些不是特定于 gstack 的）：

**自述文件.md：**
```文件：gstack-中文/document-release/SKILL.md [第 3/3 块]
规则：
- 仅改进文档中的中文翻译覆盖。
...
- 设计决策和“原因”解释仍然准确吗？
- 保守——只更新与 diff 明显矛盾的内容。架构文档描述的是不太可能经常改变的事情。

**CONTRIBUTING.md — 新贡献者冒烟测试：**
- 像新贡献者一样完成设置说明。
- 列出的命令准确吗？每一步都会成功吗？
- 测试层描述是否与当前的测试基础设施相匹配？
- 工作流程描述（开发设置、操作学习等）是否最新？
- 标记任何可能失败或让首次贡献者感到困惑的内容。

**CLAUDE.md / 项目说明：**
- 项目结构部分与实际文件树匹配吗？
- 列出的命令和脚本准确吗？
- build/test 指令是否与 package.json （或等效文件）中的内容匹配？

**任何其他 .md 文件：**
- 阅读文件，确定其目的和受众。
- 交叉引用差异来检查它是否与文件所说的任何内容相矛盾。

对于每个文件，将所需的更新分类为：

- **自动更新** - 差异明确保证了事实更正：将项目添加到表、更新文件路径、修复计数、更新项目结构树。
- **询问用户** — 叙述更改、部分删除、安全模型更改、大量重写（一节中超过 10 行），相关性不明确，添加全新的部分。

---

## 第 3 步：应用自动更新

直接使用编辑工具进行所有清晰、真实的更新。

对于每个修改的文件，输出一行摘要，描述**具体更改的内容** — 不仅仅是“更新了 README.md”，而是“README.md：将 /new-skill 添加到技能表中，将技能计数从 9 更新为 10。”
...
对于步骤 2 中确定的每个有风险或有问题的更新，请使用 AskUserQuestion：
- 上下文：项目名称、分支、哪个文档文件、我们正在审查的内容
- 具体文件决定
- __代码_0__
- 选项包括 C) 跳过 — 保持原样
...
## 第 5 步：CHANGELOG 语音润色

**重要 — 切勿破坏变更日志条目。**

这一步可以润色语音。它不会重写、替换或重新生成 CHANGELOG 内容。

曾发生过代理在应该替换现有变更日志条目的情况下替换了它们。保存了它们。这项技能绝对不能这样做。

**规则：**
1. 首先阅读整个 CHANGELOG.md。了解已经存在的内容。
2. 仅修改现有条目中的措辞。切勿删除、重新排序或替换条目。
3. 切勿从头开始重新生成 CHANGELOG 条目。该条目由 `/ship` 编写，来自实际差异和提交历史记录。它是真理的来源。你是在润色散文，而不是改写历史。
4. 如果条目看起来错误或不完整，请使用 AskUserQuestion — 不要默默地修复它。
5. 使用具有精确 `old_string` 匹配的编辑工具 — 切勿使用 Write 覆盖 CHANGELOG.md。

**如果未在此分支中修改 CHANGELOG：** 跳过此步骤。

**如果在此分支中修改了 CHANGELOG**，请检查语音条目：

- **用户测试：** 阅读每个项目符号，用户会认为“哦，太好了，我想尝试一下”吗？如果不是，重写措辞（而不是内容）。
- 以用户现在可以**做的事情为主导，而不是实现细节。
- “你现在可以......”而不是“重构......”
- 标记并重写任何读起来像提交消息的条目。
- 内部/贡献者更改属于单独的“### 对于贡献者”小节。
- 自动修复细微的语音调整。如果重写会改变含义，请使用 AskUserQuestion。

---

## 第 6 步：跨文档一致性和可发现性检查

单独审核每个文件后，执行跨文档一致性检查：

1. README 的功能/能力列表是否与 CLAUDE.md（或项目说明）描述的相匹配？
2. ARCHITECTURE 的组件列表是否与 CONTRIBUTING 的项目结构描述相匹配？
3. CHANGELOG 的最新版本与 VERSION 文件匹配吗？
4. **可发现性：** 每个文档文件都可以从 README.md 或 CLAUDE.md 访问吗？如果 ARCHITECTURE.md 存在，但 README 和 CLAUDE.md 都没有链接到它，请标记它。每个文档应该可以从两个入口点文件之一被发现。
5. 标记文档之间的任何矛盾。自动修复明显的事实不一致之处（例如，版本不匹配）。使用 AskUserQuestion 解决叙述矛盾。

---

## 第 7 步：TODOS.md 清理

这是对 `/ship` 的步骤 5.5 进行补充的第二遍。读取 `review/TODOS-format.md` （如果可用）用于规范的 TODO 项格式。

如果 TODOS.md 不存在，则跳过此步骤。

1. **尚未标记的已完成项目：** 对照未完成的 TODO 项目交叉引用差异。如果一个 TODO 显然已通过此分支中的更改完成，将其移至“已完成”部分，并附上 `**Completed:** vX.Y.Z.W (YYYY-MM-DD)`。保守一点——只用差异中清晰的证据标记项目。

2. **需要描述更新的项目：** 如果 TODO 引用了已更新的文件或组件，且发生了重大变化，其描述可能已过时。使用 AskUserQuestion 来确认 TODO 应更新、完成还是保持原样。

3. **新的推迟工作：** 检查差异中的 `TODO`、`FIXME`、`HACK` 和 `XXX` 注释。对于每一个代表有意义的延迟工作（不是一个简单的内联注释）的项目，使用 AskUserQuestion 询问是否应该在 TODOS.md 中捕获它。
...
2. 检查此分支上的 VERSION 是否已修改：

```bash
git diff <base>...HEAD -- VERSION
```

3. **如果版本未更改：** 使用 AskUserQuestion：
- 建议：选择 C（跳过），因为仅文档更改很少需要版本升级
- A) Bump PATCH (X.Y.Z+1) — 如果文档随代码更改一起更改
- B) Bump MINOR (X.Y+1.0) — 如果这是一个重要的独立版本
- C) 跳过 — 无需版本升级

4. **如果版本已经被修改：** 不要默默地跳过。相反，请检查该凹凸是否仍然涵盖该分支的全部更改范围：

a. 读取当前版本的变更日志条目。它描述了哪些特征？
b. 阅读完整的差异（`git diff <base>...HEAD --stat` 和 `git diff <base>...HEAD --name-only`）。是否有重大变化（新功能、新技能、新命令、重大重构）是当前版本的变更日志条目中未提及的？
c. **如果 CHANGELOG 条目涵盖了所有内容：** 跳过 — 输出“VERSION：已经凹凸到 vX.Y.Z，涵盖所有变化。”
d. **如果有重大未发现的更改：** 使用 AskUserQuestion 解释当前版本涵盖的更改与新增内容，并询问：
- 建议：选择 A，因为新的更改保证有自己的版本
- A) 跳转到下一个补丁 (X.Y.Z+1) — 为新的更改提供自己的版本
- B) 保留当前版本 - 将新更改添加到现有 CHANGELOG 条目
- C) 跳过 — 保留版本不变，稍后处理

关键见解：“功能 A”的版本凹凸集不应默默地吸收“功能 B”，如果功能 B 足够重要，值得拥有自己的版本条目。

---

## 第 9 步：提交和输出

**先清空检查：** 运行 `git status` （切勿使用 `-uall`）。如果没有文档文件通过之前的任何步骤进行修改，输出“所有文档都是最新的”。并退出，无需提交。

**提交：**

1. 按名称暂存修改后的文档文件（绝不是 `git add -A` 或 `git add .`）。
2. 创建单个提交：
...
1. 将现有的 PR/MR 主体读入 PID 唯一的临时文件（使用步骤 0 中检测到的平台）：
...
2. 如果临时文件已包含 `## Documentation` 部分，请将该部分替换为更新内容。如果不包含，则在末尾附加 `## Documentation` 部分。

3. 文档部分应包含 **文档差异预览** - 对于每个修改的文件，描述具体更改的内容（例如，“README.md：在技能表中添加了 /document-release，将技能计数从 9 更新为 10”）。
...
**如果是 GitLab：**
使用读取工具读取 `/tmp/gstack-pr-body-$$.md` 的内容，然后使用定界符将其传递给 `glab mr update` 以避免 shell 元字符问题：
```bash
...
6. 如果 `gh pr view` / `glab mr view` 失败（不存在 PR/MR）：跳过并显示消息“未找到 PR/MR — 跳过正文更新。”
7. 如果 `gh pr edit` / `glab mr update` 失败：警告“无法更新 PR/MR 主体 — 文档更改已提交。”并继续。
...
- **编辑前阅读。** 在修改文件之前始终阅读文件的完整内容。
- **永远不要破坏变更日志。** 仅限润色措辞。切勿删除、替换或重新生成条目。
- **永远不要默默地修改版本。** 始终询问。即使已经发生了变化，也要检查它是否涵盖了全部变更范围。
- **明确说明更改的内容。** 每次编辑都会得到一行摘要。
- **通用启发法，而不是特定于项目的。** 审计检查适用于任何存储库。
- **可发现性很重要。** 每个文档文件都应该可以从 README 或 CLAUDE.md 访问。
- **语音：友好、面向用户、不晦涩。** 像向聪明但没看过代码的人解释一样写作。