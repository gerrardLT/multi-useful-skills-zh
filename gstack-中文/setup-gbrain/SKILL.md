...
version: 1.0.0
description: |-
  为此编码代理设置 gbrain：安装 CLI、初始化
  本地 PGLite 或 Supabase 大脑，注册 MCP，捕获每个远程信任
  策略。从零到“gbrain 正在运行”只需一个命令，并且该代理
  可以调用它。使用场景：“设置 gbrain”、“连接 gbrain”、“启动
  gbrain”、“安装 gbrain”、“为本机配置 gbrain”。 （gstack）
triggers:
- setup gbrain
- install gbrain
- connect gbrain
...
在计划模式下，以下操作是允许的，因为它们用于通知计划：`$B`、`$D`、`codex exec`/`codex review`、写入 `~/.gstack/`、写入计划文件以及生成工件的 `open`。
...
选错的代价：<一句话说明什么会出错，用户会看到什么，会损失什么>
推荐：<选项> 因为 <一行原因>
...
工作量双尺度：当一个选项涉及工作量时，标记人员团队时间和 CC+gstack 时间，例如 __代码_0__。使 AI 压缩在决策时可见。
...
# /setup-gbrain — gbrain 的编码代理入门

您正在设置 gbrain (https://github.com/garrytan/gbrain)，一个持久的
知识库，在用户的本地 Mac 上，以便该编码代理（通常是
Claude Code）可以将其称为 CLI 和 MCP 工具。

**范围诚实：** 该技能的 MCP 注册步骤 (5a) 使用
`claude mcp add` 并专门针对 Claude Code。其他本地主机
（光标、Codex CLI 等）仍然会在 PATH 上获取 gbrain CLI — 他们可以
设置后手动在自己的 MCP 配置中注册 `gbrain serve` 。

**受众：** 本地 Mac 用户。 openclaw/hermes 代理通常在云中运行
具有自己的 gbrain 的 docker 容器；他们之间“共享”大脑
本地 Claude 代码只能通过共享 Postgres (Supabase) 实现。

## 用户可调用
当用户输入 `/setup-gbrain` 时，运行此技能。三种快捷方式：

- `/setup-gbrain` — 全流程（默认）
- `/setup-gbrain --repo` — 仅翻转当前存储库的每个远程策略
- `/setup-gbrain --switch` — 仅迁移引擎 (PGLite ↔ Supabase)
- `/setup-gbrain --resume-provision <ref>` — 重新输入之前中断的内容
Supabase 在轮询步骤自动配置
- `/setup-gbrain --cleanup-orphans` — 列出+删除正在进行的 Supabase 项目

自己解析调用参数——这些是技巧的散文提示，而不是
作为调度程序二进制文件实现。
...
>  同步=仅工件。无需安装任何东西；跳转到政策检查。”

`--repo`、`--switch`、`--resume-provision`、`--cleanup-orphans` 上的分支
此处调用标志并跳至匹配步骤。
...
仅当步骤 1 显示没有现有工作配置且没有快捷方式时才触发此命令
标志已通过。问题标题：“你的大脑应该住在哪里？”

选项（根据检测到的状态显示）：

- **1 — Supabase，我已经有一个连接字符串。** 云代理用户
谁的 openclaw/hermes 已经配置了一个。粘贴会话池
来自 Supabase 仪表板的 URL（设置 → 数据库 → 连接池
→ 会话）。 *提示中包含的信任表面警告：*“粘贴此
URL 为您本地的 Claude Code 提供了对您的每个页面的完整读取/write 访问权限
云代理可以看到。如果这不是您想要的信任级别，请选择 PGLite
而是局部的，并接受大脑是脱节的。”
- **2a — Supabase，自动配置一个新项目。** 您需要一个 Supabase
个人访问令牌（约 90 秒）。共享团队大脑的最佳选择。
- **2b — Supabase，手动创建。** 完成 supabase.com 注册
你自己;准备好后将 URL 粘贴回来。
- **3 — PGLite 本地。** 零帐户，约 30 秒。孤立的大脑对此
仅限苹果电脑。最适合先尝试。
- **切换**（仅当步骤 1 检测到现有引擎时）：“您已经拥有
`<engine>` 大脑。将其迁移到另一个引擎？” → 运行
`gbrain migrate --to <other>` 包裹在 `timeout 180s` (D9) 中。

不要默默地摘；触发 AskUserQuestion。

---

## 第 3 步：安装 gbrain CLI（如果缺少）
...
安装程序首先运行 D5 检测（首先探测 `~/git/gbrain`、`~/gbrain`），
然后 D19 PATH-shadow 验证（链接后 `gbrain --version` 必须匹配
安装目录 `package.json`)。在 D19 失败时，安装程​​序退出 3，并显示
清晰的修复菜单；将完整输出呈现给用户并停止。不要
继续技能 — 环境被破坏，直到用户修复 PATH。
...
获取秘密读取帮助程序，使用 `read -s` + 编辑预览收集 URL：

```bash
. ~/.claude/skills/gstack/bin/gstack-gbrain-lib.sh
``````bash
read_secret_to_env GBRAIN_POOLER_URL "粘贴会话池 URL：" \
  --echo-redacted 's#://[^@]*@#://***@#'
```
...
如果验证退出码为 3（直连 URL），则验证程序自身的
消息会解释修复方法；显示该消息并重新提示输入会话池 URL。

成功后，通过环境变量（D10，绝不通过 argv）移交给 gbrain：

```bash
GBRAIN_DATABASE_URL="$GBRAIN_POOLER_URL" gbrain init --non-interactive --json
```

然后立即执行 `unset GBRAIN_POOLER_URL GBRAIN_DATABASE_URL`。URL 现在
由 gbrain 本身在 0600 模式下保存在 `~/.gbrain/config.json` 中。

### 路径 2a（Supabase，自动配置 — D7）

在收集令牌之前，逐字显示 D11 PAT 范围披露：

> *此 Supabase 个人访问令牌授予对您 Supabase 帐户中每个项目的完全读取/写入/删除访问权限，而不仅仅是我们即将创建的 `gbrain` 项目。Supabase 目前不支持范围令牌。我们仅将此 PAT 用于：创建一个项目，轮询直至其正常运行，读取会话池 URL — 然后将其从进程内存中丢弃。该令牌在 Supabase 端仍然有效，直到您在 https://CMD_0__.com/dashboard/account/tokens 手动撤销它 — 我们建议在设置完成后立即撤销。*
...
通过 AskUserQuestion 询问 D17 层提示：“哪个 Supabase 层？”展示
免费版（2 个项目限制，7 天不活动后暂停）与专业版（$25/月，无
暂停，推荐用于实际使用）。解释该层是**组织级别**的（根据
Management API 合同）——用户根据其当前组织选择其组织
层。专业版可能要求他们首先在 supabase.com 升级组织。

列出组织，选择一个（如果有多个，则询问用户问题）：

```bash
orgs=$(~/.claude/skills/gstack/bin/gstack-gbrain-supabase-provision list-orgs --json)
```

如果 `.orgs` 数组为空，则显示：“您的 Supabase 帐户没有
组织。请在 https:///supabase.com/dashboard 创建一个，然后重新运行
`/setup-gbrain`。”停止。

询问用户一个区域（默认 `us-east-1`；有效值为
Supabase Management API 中的 18 个枚举值 — 列出一些常见的值，让
他们选择“其他”以获取完整列表）。
...
> “设置完成。请在 https://CMD_0__.com/dashboard/account/tokens 撤销您粘贴的 PAT — 我们已将其从内存中丢弃，不再需要它。gbrain 项目将继续工作，因为它使用自己的嵌入式数据库密码。”

### 路径 2b（Supabase，手动）

引导用户完成 supabase.com 步骤：
1. 登录 https://CMD_0__.com/dashboard
2. 点击“New Project”，将其命名为 `gbrain`，选择一个区域，复制生成的
数据库密码（您需要它来粘贴回来？不 - 它嵌入在
我们接下来收集的池化器 URL 中）
3. 等待大约 2 分钟让项目初始化
4. 设置 → 数据库 → 连接池 → 会话 → 复制 URL（端口
   6543)

然后遵循与路径 1 相同的秘密读取 + 验证 + 初始化流程。
...
```bash
# 从 PGLite 切换到 Supabase，先收集 URL（路径 1 流程），然后：
timeout 180s gbrain migrate --to supabase --url "$URL" --json
# 从 Supabase 切换到 PGLite：
timeout 180s gbrain migrate --to pglite --json
```

如果 `timeout` 返回 124（超时退出代码）：显示 D9 消息
（“迁移在 3 分钟内未完成 — 另一个 gstack 会话可能
锁定了源 brain。关闭其他工作区并重新运行
__代码_0__。您原来的 brain 未受影响。”）。停止。
...
如果状态为 `ok` 或 `warnings`，则继续。任何其他状态 → 显示完整
doctor 输出并停止。
...
仅当 `which claude` 能解析时执行。询问：“为 Claude Code 提供一个类型化的工具接口
用于 gbrain 吗？（推荐是）”

如果是，请在 **用户范围** 注册，并使用 gbrain 的 **绝对路径**
二进制文件。用户范围使 MCP 在此机器上的每个 Claude Code 会话中
可用，而不仅仅是当前工作区。绝对路径避免了当 Claude Code
将 `gbrain serve` 作为子进程生成时的 PATH 问题。
...
如果用户在之前的运行中已经进行了本地范围的注册，
首先删除它，这样两个范围就不会冲突：
```bash
claude mcp remove gbrain 2>/dev/null || true
```

如果 `claude` 不在 PATH 中：发出“MCP 注册已跳过 - 此技能
针对 Claude Code；请在代理的 MCP 配置中手动注册 `gbrain serve`。”继续步骤 6。

**用户注意：**已经打开的 Claude Code 会话不会
选择新的 MCP 工具直至重新启动。告诉他们：“重新启动任何打开的
Claude Code 会话以查看 `mcp__gbrain__*` 工具 - 它们在会话开始时加载，
而不是在会话中间。”

---

## 第 6 步：每个远程策略（D3 三元组，门控仓库导入）

如果我们在带有 `origin` 远程仓库的 git 仓库中，请检查策略：
...
`gbrain embed --stale &` 在后台运行。
- `read-only` → 完全跳过导入（此层由未来的自动导入钩子
+ 通过 gbrain 解析器注入强制执行，不在此处）。
- `deny` → 什么都不做。
- `unset` → 询问用户问题：“`<normalized-remote>` 应该如何与
brain 交互？”
- `read-write` — 代理可以从此仓库搜索并写入新页面
- `read-only` — 代理可以搜索但不能写入
- `deny` — 根本没有交互
- `skip-for-now` — 不坚持，下次再问
...
如果在 git 仓库之外或没有 origin 远程仓库：请跳过此步骤并添加注释。

对于 `/setup-gbrain --repo` 调用，仅执行步骤 6 并退出。

---

## 第 7 步：提供 gstack-brain-sync + 将其连接到 gbrain

单独的 AskUserQuestion：“同时将您的 gstack 会话内存（学习、
计划、回顾）同步到 gbrain 可以跨机器索引的私有 git 仓库吗？”

选项：
- 是的，完全同步（所有内容均列入许可名单）
- 是的，仅限工件（计划、设计、回顾 - 跳过行为数据）
- 不，谢谢
...
# 或者如果用户选择了 yes-full 则为 "full"
```

然后将 Brain 仓库连接到 gbrain，以便可以从任何地方搜索其内容
gbrain 客户端（此 Claude Code 会话、未来的 Mac、可选的云代理）。
助手创建 `~/.gstack/` 的 `git worktree`，并将其注册为
用户 gbrain（Supabase 或 PGLite）上的联合源，并运行
初始 `gbrain sync`。仅限本地 Mac。无需云代理。后续的
技能运行通过现有的技能端推送挂钩触发增量同步。

首先从 `~/.gbrain/config.json` 中捕获数据库 URL 并将其显式传递，
这样接线对于任何其他进程重写都是稳健的
`~/.gbrain/config.json` 中间同步（例如，并发 `gbrain init` 运行
在机器上的其他地方）：
...
或者还没有 `~/.gstack/.git`），这样用户就会看到失败而不是默默地
最终得到一个没有连线的 brain。在非零退出时，显示助手的
根据技能规则输出并停止 - 跨机器搜索将无法工作，直到
先决条件被修复。

---

## 第 8 步：在 CLAUDE.md 中保留 `## GBrain Configuration`

在 CLAUDE.md 中查找并替换（或附加）此部分：
...
确认往返操作。失败时，显示 `gbrain doctor --json` 输出
并通过 NEEDS_CONTEXT 升级停止。
...
```bash
# 列出用户的 Supabase 项目（用户必须通过他们自己的 shell 管道传输此内容
# 以进行审查；我们不依赖存储的 PAT）。
...
解析响应，识别任何以 `gbrain` 开头的项目，其
`ref` 与用户活动的 `~/.gbrain/config.json` 池化器 URL 不匹配。
对于每个孤立项目，向每个项目询问用户问题：“删除孤立项目
`<ref>`（`<name>`，创建于 `<created_at>`）？” — 绝不批量；每个项目
确认是一扇单向门。
...
未经第二次明确确认，绝不删除活动 brain。

最后：`unset SUPABASE_ACCESS_TOKEN`。撤销提醒。

---

## 遥测 (D4)

序言的遥测块在退出时记录技能 success/failure。当
发出事件时，将这些枚举的分类值添加到
遥测有效负载（安全 - 没有自由格式的秘密，绝不包含 URL 或 PAT）：

- __代码_0__：__代码_1__|__代码_0__|
__代码_0__|__代码_0__|__代码_0__|
__代码_0__|__代码_0__|__代码_0__|
__代码_0__
- __代码_0__：__代码_1__|`no`（D5 重用）|`skipped`（已存在）
- __代码_0__：__代码_1__|__代码_0__|__代码_0__
- __代码_0__：__代码_1__|__代码_0__|__代码_0__|
__代码_0__|`n/a` （在 git 仓库之外）

绝不传递 `SUPABASE_ACCESS_TOKEN`、`DB_PASS`、`GBRAIN_POOLER_URL`、
`GBRAIN_DATABASE_URL` 或任何 `postgresql://` 子字符串到遥测数据
调用。`test/skill-validation.test.ts` 中的 CI grep 测试在构建时强制执行
此规则。
...
- **每个秘密都有一个规则。** PAT、DB_PASS、池化器 URL：仅限环境变量，
我们绝不通过 argv，绝不记录，也绝不保存到磁盘。唯一长期
保存池化器 URL 的文件是 `~/.gbrain/config.json`，通过 gbrain 自己的
`init` 以模式 0600 写入 — 这是 gbrain 的规则，不是我们的。
- **停止点很严格。** Gbrain doctor 不健康，D19 PATH 阴影，D9
迁移超时、冒烟测试失败——每一个都是一个停止点。不要掩盖问题。
- **并发运行锁定。** 在技能开始时，`mkdir ~/.gstack/.setup-gbrain.lock.d`
（原子操作）。如果 mkdir 失败，则中止：“另一个 `/setup-gbrain` 实例
正在运行。等待它完成，或者如果确定它已过时则执行 `rm -rf ~/.gstack/.setup-gbrain.lock.d`。”在正常退出和 SIGINT 陷阱中释放。
- **CLAUDE.md 是审计跟踪。** 始终在步骤 8 后更新它
设置成功。
```