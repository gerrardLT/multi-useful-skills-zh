...
version: 1.0.0
description: |-
  保存工作环境。捕获 git 状态、做出的决策和剩余工作，
  以便后续任何会议都能无缝衔接，不丢失任何进度。
  当用户要求“保存进度”、“保存状态”、“上下文保存”或
  “保存我的工作”时触发。与 /context-restore 配对，用于稍后恢复。
  前身为 /checkpoint — 重命名是因为 Claude Code 将 /checkpoint 视为
  当前环境中的原生回滚别名，从而掩盖了此技能。
  （gstack）
allowed-tools:
- Bash
- Read
...
如果用户在计划模式下调用技能，则该技能优先于通用计划模式行为。**将技能文件视为可执行指令，而不是参考。** 从 Step 0 开始逐步执行；第一个 AskUserQuestion 是工作流进入计划模式的入口，而不是违反它。AskUserQuestion 满足计划模式的回合结束要求。到达停止点后，立即停止。不要继续工作流程或在那里调用 ExitPlanMode。执行标记为“PLAN MODE EXCEPTION — ALWAYS RUN”的命令。仅在技能工作流程完成后，或者用户告诉您取消技能或离开计划模式时，才调用 ExitPlanMode。

如果 `PROACTIVE` 是 `"false"`，则不要自动调用或主动建议技能。如果某项技能看起来有用，请询问：“我认为 /skillname 可能会有所帮助 - 希望我运行它吗？”

如果 `SKILL_PREFIX` 是 `"true"`，则建议使用 `/gstack-*` 名称调用。磁盘路径保持为 `~/.claude/skills/gstack/[skill-name]/SKILL.md`。

如果输出显示 `UPGRADE_AVAILABLE <old> <new>`：读取 `~/.claude/skills/gstack/gstack-upgrade/SKILL.md` 并遵循“内联升级流程”（如果配置为自动升级则执行，否则使用 4 个选项询问用户，如果拒绝则写入暂停状态）。
...
您是一名**主管工程师，负责记录细致的会议记录**。你的工作是
捕捉完整的工作环境——正在做什么、做了什么决定、
剩下的——以便任何未来的会话（即使在不同的分支或工作区）
可以通过 `/context-restore` 恢复而不丢失节拍。

**硬门：** 不要实施代码更改。该技能仅捕获状态。
...
使用它作为标题。否则，从当前作品推断出标题。

如果用户键入 `/context-save resume` 或 `/context-save restore`，请告诉他们：
“使用 `/context-restore` 来代替——保存和恢复现在是独立的技能了。”
...
使用收集到的状态加上您的对话历史记录，生成涵盖以下内容的摘要：

1. **正在进行的工作** — 高级目标或功能
2. **做出的决定** — 架构选择、权衡、选择的方法以及原因
3. **剩余工作** — 具体的后续步骤，按优先顺序排列
4. **注释** — 未来会话需要了解的任何内容（陷阱、被阻止的项目、
开放性问题，尝试过但不起作用的事情）

如果用户提供了标题，请使用它。否则，推断一个简洁的标题（3-6 个单词）
从正在完成的工作来看。
...
如果无法确定持续时间，请省略 `session_duration_s` 字段
保存的文件。

### 第 4 步：写入保存的上下文文件

在 bash 中计算路径（不在 LLM 提示符中），因此用户提供的标题不能
将 shell 元字符注入任何后续命令中。消毒剂是一种
允许名单：只有 `a-z 0-9 - .` 存活。
...
# 转小写，将空白折叠为连字符，剥离至允许名单，限制长度。
TITLE_SLUG=$(printf '%s' "$RAW" | tr '[:upper:]' '[:lower:]' | tr -s ' \t' '-' | tr -cd 'a-z0-9.-' | cut -c1-60)
TITLE_SLUG="${TITLE_SLUG:-untitled}"
```# 防冲突文件名：如果 `${TIMESTAMP}-${SLUG}.md` 已存在（同一秒内使用相同标题重复保存），则追加一个短随机后缀。文件名是仅追加的——永不覆盖。
...
磁盘上的目录名称是 `checkpoints/` （不是 `contexts/`）——这是一个旧版本保留的路径，以便现有的已保存文件保持可加载状态。用户永远看不到它。

将文件写入上面打印的 `$FILE` 路径（使用确切的字符串——不要在 LLM 层重建它）。
...
session_duration_s: {计算出的持续时间，如果未知则省略}
files_modified:
  - path/to/file1
  - path/to/file2
---
...
{注意事项、阻塞项、待解决问题、尝试过但未成功的方法}
```

`files_modified` 列表来自 `git status --short`（包括已暂存和未暂存的修改文件）。使用相对于存储库根目录的路径。
...
if [ -d "$CHECKPOINT_DIR" ]; then
  echo "CHECKPOINT_DIR=$CHECKPOINT_DIR"
  # 使用 find + sort 代替 ls -1t：文件名 YYYYMMDD-HHMMSS 前缀是规范的排序依据（在复制/rsync 时稳定；mtime 不稳定），并且空结果行为干净（无文件 → 无输出，不会回退到列出当前目录）。
...
如果用户传递 `--all`（例如 `/context-save list --all`），则显示来自**所有分支**的上下文。

读取每个文件的 frontmatter 以提取 `status`、`branch` 和 `timestamp`。从文件名中解析标题（时间戳后面的部分）。
...
如果没有保存的上下文，请告诉用户：“还没有保存的上下文。运行 `/context-save` 来保存您当前的工作状态。”
...
- **始终在 frontmatter 中包含分支名称** - 这对于跨分支恢复至关重要。
- **保存的文件只能追加。** 切勿覆盖或删除现有文件。每次保存都会创建一个新文件。
- **推断，不要询问。** 使用 git 状态和对话上下文来填写文件。仅当标题确实无法推断时才使用 AskUserQuestion。
- **这是一个 gstack 技能，而不是内置的 Claude Code 功能。** 当用户输入 `/context-save` 时，通过技能工具调用该技能。旧的 `/checkpoint` 名称与 Claude Code 的本机 `/rewind` 别名冲突——重命名解决了这个问题。