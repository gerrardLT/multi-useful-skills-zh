...
description: |-
  ...
  打开一个可见的浏览器窗口，您可以在其中实时观看每个操作。
  侧边栏显示实时活动提要和聊天。内置反机器人隐身功能。
  当要求“打开 gstack 浏览器”、“启动浏览器”、“连接 chrome”时使用，
  “打开 chrome”、“真实浏览器”、“启动 chrome”、“侧面板”或“控制我的浏览器”。
  语音触发器（语音到文本别名）：“向我显示浏览器”。
triggers:
- open gstack browser
- launch chromium
- show me the browser
...
回答后，记录尽力而为：
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"open-gstack-browser","question_id":"<id>","question_summary":"<简短描述>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
启动 GStack 浏览器 — AI 控制的 Chromium，带有侧边栏扩展，
反机器人隐身和定制品牌。您可以实时看到每一个动作。
...
在连接之前，请终止所有过时的浏览服务器并清理锁定文件
可能因崩溃而持续存在。这可以防止“已连接”错误
正面和 Chromium 配置文件锁定冲突。
...
这将以有头模式启动 GStack 浏览器（已重命名为 Chromium）：
- 一个可见的窗口供您观看（不是常规的 Chrome — 它保持不变）
- gstack 侧边栏扩展通过 `launchPersistentContext` 自动加载
- 反机器人检测补丁（Google 和 NYTimes 等网站无需验证码即可运行）
- Dock/菜单栏中的自定义用户代理和 GStack 浏览器品牌标识
- 用于聊天命令的侧边栏代理进程

`connect` 命令会自动发现 gstack 安装目录中的扩展。它始终使用端口 **34567**，以便扩展可以自动连接。

连接后，会向用户打印完整输出。请确认您在输出中看到了 `Mode: headed`。

如果输出显示错误或模式不是 `headed`，请运行 `$B status` 并在继续之前与用户共享输出。
...
端口应该是 **34567**。如果不同，请记下 - 用户可能需要它来连接侧面板。

同时查找扩展路径，以便在用户需要手动加载时可以提供帮助：
...
> Chrome 是通过 gstack 控件启动的。你应该能看到 Playwright 的 Chromium
> （不是普通的 Chrome），页面顶部有一条金色闪光线。
>
> 侧面板扩展应该会自动加载。打开它：
> 1. 在工具栏中查找 **拼图图标**（扩展） - 如果扩展加载成功，它可能已经显示 gstack 图标
> 2. 点击 **拼图** → 找到 **gstack browser** → 点击 **图钉图标**
> 3. 单击工具栏中固定的 **gstack 图标**
> 4. 侧面板应在右侧打开，显示实时活动流
>
> **端口：** 34567（自动检测 — 扩展程序在 Playwright 控制的 Chrome 中运行）。

选项：
- A) 我可以看到侧面板 - 开始吧！
- B) 我可以看到 Chrome，但找不到扩展程序
- C) 出了点问题

如果选 B：告诉用户：

> 该扩展在启动时加载到 Playwright 的 Chromium 中，但有时它不会立即出现。尝试以下步骤：
>
> 1. 在地址栏中输入 `chrome://extensions`
> 2. 查找 **“gstack browser”** — 它应该已列出并启用
> 3. 如果它存在但未固定，请返回任意页面，单击拼图块图标，然后将其固定
> 4. 如果根本没有列出，请单击 **“加载已解压的扩展程序”** 并导航至：
...
> 加载后，将其固定并单击图标打开侧面板。
>
> 如果侧面板徽章保持灰色（已断开连接），请单击 gstack 图标并手动输入端口 **34567**。

如果选 C：

1. 运行 `$B status` 并显示输出
2. 如果服务器运行状况不佳，请重新运行步骤 0 清理 + 步骤 1 连接
3. 如果服务器正常但浏览器不可见，请尝试 `$B focus`
4. 如果失败，请询问用户他们看到了什么（错误消息、黑屏等）

## 第 4 步：演示

用户确认侧面板正常工作后，运行一个快速演示：
...
告诉用户：“检查侧面板 - 您应该看到 `goto` 和 `snapshot` 命令出现在活动流中。Claude 运行的每个命令都会实时显示在这里。”

## 第 5 步：侧边栏聊天

在活动流演示之后，告诉用户侧边栏聊天功能：

> 侧面板还有一个 **聊天选项卡**。尝试输入一条消息，例如“截取快照并描述此页面。”侧边栏代理（子 Claude 实例）会在浏览器中执行您的请求 - 您将看到命令在活动发生时出现在活动流中。
>
> 侧边栏代理可以导航页面、单击按钮、填写表单和阅读内容。每个任务最多 5 分钟。它在一个隔离的会话中运行，所以不会干扰这个 Claude 代码窗口。

## 第 6 步：下一步是什么

告诉用户：

> 你都准备好了！您可以使用已连接的 Chrome 执行以下操作：
>
> **实时观看 Claude 工作：**
> - 运行任何 gstack 技能（`/qa`、`/design-review`、`/benchmark`）并观看每个操作都发生在可见的 Chrome 窗口 + 侧面板活动流中
> - 无需导入 cookie — Playwright 浏览器共享其自己的会话
>
> **直接控制浏览器：**
> - **侧边栏聊天** — 在侧面板中输入自然语言，侧边栏代理会执行它（例如，“填写登录表单并提交”）
> - **浏览命令** — `$B goto <url>`、`$B click <sel>`、`$B fill <sel> <val>`、`$B snapshot -i` — 在 Chrome + 侧面板中全部可见
>
> **窗口管理：**
...
>
> **有头模式下的技能是什么样的：**
> - `/qa` 在可见浏览器中运行完整的测试套件 - 您可以看到每个页面加载、每次点击、每次断言
> - `/design-review` 在真实浏览器中截取屏幕截图 - 与您看到的像素完全相同
> - `/benchmark` 测量有头浏览器的性能

然后继续执行用户要求执行的操作。如果他们没有指定任务，询问他们想测试或浏览什么。
```