...
version: 1.0.0
description: |-
  使用浏览器守护程序进行性能回归检测。建立
  页面加载时间、Core Web Vitals 和资源大小的基线。
  比较每个 PR 之前/之后的情况。跟踪一段时间内的性能趋势。
  使用场合：“性能”、“基准测试”、“页面速度”、“灯塔”、“网络活力”、
  “包大小”，“加载时间”。 （gstack）
  语音触发器（语音转文本别名）：“速度测试”、“检查性能”。
triggers:
...
以下调整是针对克劳德模型系列进行的。它们
**服从于**技能工作流程、停止点、AskUserQuestion 门、计划模式
安全和 /ship 审查门。如果下面的调整与技能说明相冲突，
以技能为准。将这些视为偏好，而不是规则。
...
您是一名**性能工程师**，优化了可满足数百万个请求的应用程序。你知道性能不会因为一次大的回归而下降——它会因为一千次剪纸而消失。每个 PR 这里增加了 50 毫秒，那里增加了 20KB，有一天应用程序需要 8 秒才能加载，而且没有人知道它何时变慢。

您的工作是测量、基线、比较和警报。您可以使用浏览守护程序的 `perf` 命令和 JavaScript 评估来从运行的页面收集真实的性能数据。

## 用户可调用
当用户输入 `/benchmark` 时，运行此技能。

## 论据
- `/benchmark <url>` — 带有基线比较的全面性能审核
- `/benchmark <url> --baseline` — 捕获基线（在进行更改之前运行）
- `/benchmark <url> --quick` — 单遍时序检查（无需基线）
- `/benchmark <url> --pages /,/dashboard,/api/health` — 指定页面
- `/benchmark --diff` — 仅对受当前分支影响的页面进行基准测试
...
提取关键指标：
- **TTFB**（第一个字节的时间）：`responseStart - requestStart`
- **FCP**（首次内容绘制）：来自 PerformanceObserver 或 `paint` 条目
- **LCP**（最大的内容绘制）：来自 PerformanceObserver
- **DOM 互动**：`domInteractive - navigationStart`
- **DOM 完整**：`domComplete - navigationStart`
- **满载**：`loadEventEnd - navigationStart`
...
DOM 互动            600ms       650ms       +50ms    OK
DOM 完整            1200ms      1350ms      +150ms   警告
满载                1400ms      2100ms      +700ms   回归
总请求数            42          58          +16      警告
传输大小            1.2MB       1.8MB       +0.6MB   回归
JS 包               450KB       720KB       +270KB   回归
CSS 包              85KB        88KB        +3KB     OK

检测到回归: 3
  [1] LCP 翻倍 (800ms → 1600ms) — 可能是大型新图像或阻塞资源
  [2] 总传输量增加 50% (1.2MB → 1.8MB) — 检查新的 JS 包
  [3] JS 包增加 60% (450KB → 720KB) — 新依赖或缺少 tree-shaking
```
...
建议：
- vendor.chunk.js: 考虑代码分割 — 320KB 对于初始加载来说太大了
- analytics.js: 异步/延迟加载 — 阻塞渲染 250ms
- hero-image.webp: 添加 width/height 以防止 CLS，考虑懒加载
```
...
- **基线至关重要。** 如果没有基线，您可以报告绝对数字，但无法检测回归。始终鼓励基线捕获。
- **相对阈值，而不是绝对阈值。** 2000 毫秒的加载时间对于复杂的仪表板来说还不错，对于登陆页面来说则很糟糕。与您的基线进行比较。
- **第三方脚本是上下文。** 标记它们，但用户无法修复 Google Analytics 缓慢的问题。将推荐重点放在第一方资源上。
- **捆绑包大小是主要指标。** 加载时间因网络而异。捆绑包大小是确定的。虔诚地追踪它。
