...
version: 1.0.0
description: |-
  恢复之前由 /context-save 保存的工作上下文。加载最新的
  保存状态（默认情况下跨所有分支），以便您可以从上次
  停止的地方继续——甚至跨越 Conductor 工作空间交接。
  当被要求“恢复”、“恢复上下文”、“我在哪里”或
  “从我停下来的地方继续”时触发。与 /context-save 配对。
  以前的 /checkpointresume — 重命名是因为 Claude Code 将 /checkpoint
  视为当前环境中的本机倒带别名。 （gstack）
allowed-tools:
- Bash
- Read
- Glob
...
如果 `SKILL_PREFIX` 是 `"true"`，则建议使用 `/gstack-*` 名称进行 /invoke。磁盘路径保留为 `~/.claude/skills/gstack/[skill-name]/SKILL.md`。
...
您是一名**主管工程师，正在阅读同事细致的会议笔记**
准确地从他们停下的地方继续。您的工作是加载最近保存的
上下文并清晰地呈现出来，以便用户可以继续工作而不会失去节奏。

**硬门：** 不要实施代码更改。该技能仅读取已保存
上下文文件并呈现摘要。

**默认：跨所有分支加载最近保存的上下文。** 这是
故意与 `/context-save list` 不同，后者默认为当前
分支。 `/context-restore` 用于 Conductor 工作区切换 — 保存的上下文
可以从另一个分支恢复。

**不要按当前分支过滤候选集。** `list` 流程不会
那; `/context-restore` 没有。
...
- `/context-restore <title-fragment-or-number>` → 加载特定的已保存上下文
- `/context-restore list` → 告诉用户“使用 `/context-save list` — 列表
位于保存端”并退出。这里没有模式检测。
...
if [ ! -d "$CHECKPOINT_DIR" ]; then
  ...
  # 使用 find + sort 代替 ls -1t。两个原因：
  # 1. 规范顺序是文件名 YYYYMMDD-HHMMSS 前缀（在复制/rsync 时稳定）。文件系统 mtime 会漂移且不是权威的。
  # 2. 在 macOS 上，`find ... | xargs ls -1t` 在零结果时会回退到列出 cwd。`sort -r` 在空输入时会干净地返回无。
  # 限制为最近 20 个：拥有 10k 个保存文件的用户不应仅列出它们就耗尽上下文窗口。/context-save list 处理分页。
...
**候选者包含目录中的每个 `.md` 文件，无论分支如何**
（分支记录在frontmatter中，这里不用于过滤）。这
启用 Conductor 工作区切换。

### 第 2 步：加载正确的文件

- 如果用户指定了标题片段或编号：在其中查找匹配的文件
候选人。
- 否则：加载 **上面 `sort -r` 返回的第一个文件** — 即
最新的 `YYYYMMDD-HHMMSS` 前缀，这是规范的“最新”。
...
════════════════════════════════════════
标题:       {title}
分支:      {branch from frontmatter}
保存时间:       {timestamp, human-readable}
持续时间:    上次会话持续 {formatted duration}（如果可用）
状态:      {status}
════════════════════════════════════════
...
如果当前分支与保存的上下文分支不同，请注意：
“此上下文已保存在分支 `{branch}` 上。您当前位于
__代码_0__。在继续之前，您可能需要切换分支。”

### 第 3 步：提供后续步骤

演示后，通过 AskUserQuestion 提问：

- A) 继续处理剩余项目
- B) 显示完整保存的文件
-C）只需要上下文，谢谢
```如果是 A，总结第一个剩余的工作项目并建议从那里开始。

---

## 如果不存在保存的上下文

如果步骤 1 打印 `NO_CHECKPOINTS`，请告诉用户：

“尚未保存上下文。首先运行 `/context-save` 来保存当前的工作
状态，然后 `/context-restore` 就会找到它。”

---

## 重要规则

- **切勿修改代码。** 该技能仅读取保存的文件并呈现它们。
- **默认情况下始终在所有分支中搜索。** 跨分支恢复是
关键点。仅当用户通过分支明确询问时才按分支过滤
标题片段匹配恰好是特定于分支的。
- **“最新”是指文件名 `YYYYMMDD-HHMMSS` 前缀**，而不是
`ls -1t`（文件系统 mtime）。文件名在文件系统中是稳定的
操作；mtime 不是。
- **这是一个 gstack 技能，而不是内置的 Claude Code。** 当用户输入时
`/context-restore`，通过技能工具调用该技能。