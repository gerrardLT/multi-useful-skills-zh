...
2. **Read \`.claude/skills/review/design-checklist.md\`.** If the file cannot be read, skip design review with a note: "Design checklist not found 鈥?skipping design review."
...
### Quick (\`--quick\`)
仅检查首页和 2 个关键页面。包含第一印象、设计系统提取和精简检查清单。这是获得设计评分的最快路径。
...
3. Write the **First Impression** using this structured critique format:
   ...
   - "The first 3 things my eye goes to are: **[1]**, **[2]**, **[3]**." (hierarchy check 鈥?are these the 3 things the designer intended? If not, the visual hierarchy is lying.)
...
\`\`\`
Goodwill: 70 鈻堚枅鈻堚枅鈻堚枅鈻堚枅鈻堚枅鈻堚枅鈻堚枅鈻堚枅鈻堚枅鈻堚枅鈻戔枒鈻戔枒鈻戔枒鈻戔枒鈻戔枒
  Step 1: Login page        70 鈫?75  (+5 obvious primary action)
  Step 2: Dashboard          75 鈫?60  (-15 interstitial tour popup)
  Step 3: Settings           60 鈫?50  (-10 format punishment on phone)
  Step 4: Billing            50 鈫?35  (-15 hidden pricing info)
  FINAL: 35/100 鈿狅笍 CRITICAL UX DEBT
\`\`\`
...
11. **Show screenshots to the user.** After every \`$B screenshot\`, \`$B snapshot -a -o\`, or \`$B responsive\` command, use the Read tool on the output file(s) so the user can see them inline. For \`responsive\` (3 files), Read all three. This is critical 鈥?without it, screenshots are invisible to the user.`;
}
...
_REPO_ROOT=$(git rev-parse --show-toplevel) || { echo "ERROR: not in a git repo" >&2; exit 1; }
codex exec "For this product approach, provide: a visual thesis (one sentence 鈥?mood, material, energy), a content plan (hero 鈫?support 鈫?detail 鈫?CTA), and 2 interaction ideas that change page feel. Apply beautiful defaults: composition-first, brand-first, cardless, poster not document. Be opinionated." -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="medium"' --enable web_search_cached < /dev/null 2>"$TMPERR_SKETCH"
\`\`\`
...
将 Codex 输出放在 \`CODEX 表示（设计草图）:\` 标题下，将子代理输出放在 \`CLAUDE SUBAGENT（设计方向）:\` 标题下。
...
HARD RULES 鈥?first classify as MARKETING/LANDING PAGE vs APP UI vs HYBRID, then flag violations of the matching rule set:
...
2. Missing states: loading, empty, error, success, partial 鈥?which are unspecified?
3. User journey: what's the emotional arc? Where does it break?
4. Specificity: does the plan describe SPECIFIC UI ("48px S枚hne Bold header, #1a1a1a on white") or generic patterns ("clean modern card-based layout")?
...
Be opinionated. Be specific. Do not hedge. This is YOUR design direction 鈥?own it.`;
...
鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺?
  ...
  1. Brand unmistakable in first screen?   鈥?      鈥?     鈥?
  2. One strong visual anchor?             鈥?      鈥?     鈥?
  3. Scannable by headlines only?          鈥?      鈥?     鈥?
  4. Each section has one job?             鈥?      鈥?     鈥?
  5. Cards actually necessary?             鈥?      鈥?     鈥?
  6. Motion improves hierarchy?            鈥?      鈥?     鈥?
  7. Premium without decorative shadows?   鈥?      鈥?     鈥?
  鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€ 鈹€鈹€鈹€鈹€鈹€鈹€鈹€ 鈹€鈹€鈹€鈹€鈹€鈹€鈹€ 鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€鈹€
  Hard rejections triggered:               鈥?      鈥?     鈥?
鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺愨晲鈺?
\`\`\`
...
- "Codex and I agree on X. Codex suggested Y where I'm proposing Z 鈥?here's why..."` : `
**Synthesis 鈥?Litmus scorecard:**
...
- If Claude subagent also fails: "Outside voices unavailable 鈥?continuing with primary review."

将 Codex 输出放在 \`CODEX 表示（设计${isPlanDesignReview ? '批评' : isDesignReview ? '源码审计' : '方向'}）:\` 标题下。
将子代理输出放在 \`CLAUDE SUBAGENT（设计${isPlanDesignReview ? '完整性' : isDesignReview ? '一致性' : '方向'}）:\` 标题下。
${synthesisSection}
...
// 鈹€鈹€鈹€ Design Hard Rules (OpenAI framework + gstack slop blacklist) 鈹€鈹€鈹€
...
- NEVER put labels inside form fields as the only label (placeholder-as-label pattern 鈥?labels must be visible when the field has content)
...
Commands:
- \`$D generate --brief "..." --output /path.png\` 鈥?generate a single mockup
- \`$D variants --brief "..." --count 3 --output-dir /path/\` 鈥?generate N style variants
- \`$D compare --images "a.png,b.png,c.png" --output /path/board.html --serve\` 鈥?comparison board + HTTP server
- \`$D serve --html /path/board.html\` 鈥?serve comparison board and collect feedback via HTTP
- \`$D check --image /path.png --brief "..."\` 鈥?vision quality gate
- \`$D iterate --session /path/session.json --feedback "..." --output /path.png\` 鈥?iterate
...
if [ -f "$_TASTE_PROFILE" ]; then
  ...
  # Confidence decays 5% per week of inactivity 鈥?computed at read time.
...
this time 鈥?I'll proceed, but want me to update the taste profile, or treat this
as a one-off?"
...
the legacy approved.json aggregate 鈥?\`${ctx.paths.binDir}/gstack-taste-update\`
will migrate it to schema v1 on the next write.`;
}

// 鈹€鈹€鈹€ UX Behavioral Foundations (Krug + HCI research) 鈹€鈹€鈹€
